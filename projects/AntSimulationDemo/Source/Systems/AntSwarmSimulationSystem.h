...
class AntSwarmSimulationSystem : public SimulationSystem<AntSwarmSimulation> {
  public:
    ...
    void Update(AntSwarmSimulation &simulation, ProjectRuntimeContext &context) override {
        ...
        for (AntAgent &ant : simulation.Agents()) {
            StepAnt(simulation, ant, context.registry, context.tileMap, context.projectConfig,
        ...
        DenseCircleSolver::Resolve(simulation.Agents(), physicsConfig, [&](glm::vec2 from, glm::vec2 to) {
  ...
  private:
    ...
    void ApplySettingsFromSwarm(AntSwarmSimulation &simulation, Registry &registry) {
        ForEachProjectEntity<AntSwarmComponent>(registry, [&](AntSwarmComponent &swarm) {
    ...
    void HideDepletedFoodSources(Registry &registry) {
        ForEachProjectEntity<FoodSourceComponent>(registry, [&](Entity entity, FoodSourceComponent &food) {
            (void)food;
            if (GetFoodAmount(entity) > 0) {
                return;
            }
    ...
    void EnsureFields(AntSwarmSimulation &simulation, const TileMap &tileMap, const ProjectConfig &projectConfig) {
    ...
    void RefreshPersistentHomeMarkers(AntSwarmSimulation &simulation, Registry &registry, const TileMap &tileMap,
                                      const ProjectConfig &projectConfig) {
        ForEachProjectEntity<AntColonyComponent, TransformComponent>(
            registry, [&](Entity colony, AntColonyComponent &colonyComponent, TransformComponent &transform) {
                (void)colonyComponent;
                (void)transform;
                ...
                FieldGridSolver::ApplyDeposit(simulation.Fields()["home"], tileMap, projectConfig.simulation, command);
            });
    }

    void SpawnFromColonies(AntSwarmSimulation &simulation, Registry &registry, const TileMap &tileMap,
                           double deltaTime) {
        ...
        ForEachProjectEntity<AntColonyComponent, TransformComponent>(
            registry, [&](Entity colony, AntColonyComponent &colonyComponent, TransformComponent &transform) {
                ...
                while (simulation.Count() < static_cast<std::size_t>(std::max(maxAgents, 0)) &&
                    ...
                    ant.role = Random01(simulation) < simulation.Settings().explorerRatio ? AntRole::Explorer
                                                                                          : AntRole::Follower;
    ...
    void RemoveDeadAgents(AntSwarmSimulation &simulation) {
        ...
        agents.erase(
            std::remove_if(agents.begin(), agents.end(), [](const AntAgent &ant) { return ant.energy <= 0.0f; }),
            agents.end());
    }

    int GetConfiguredMaxAgents(const AntSwarmSimulation &simulation, Registry &registry, int fallbackMaxAgents) const {
        (void)simulation;

        ForEachProjectEntity<AntSwarmComponent>(registry,
                                                [&](AntSwarmComponent &swarm) { fallbackMaxAgents = swarm.maxAgents; });

        return fallbackMaxAgents;
    }

    void StepAnt(AntSwarmSimulation &simulation, AntAgent &ant, Registry &registry, const TileMap &tileMap,
                 const ProjectConfig &projectConfig, float deltaTime) {
        ant.age += deltaTime;
        ...
        const glm::vec2 moveDirection =
            TurnTowards(ant.direction, targetDirection, simulation.Settings().maxTurnRadiansPerSecond * deltaTime);
    ...
    bool IsBlocked(const AntSwarmSimulation &simulation, const AntAgent &ant) const {
        const double markerTime = simulation.Settings().markerDistance / std::max(simulation.Settings().speed, 0.001f);
        return ant.timeSinceMarker > markerTime * 10.0 || ant.stuckTime > simulation.Settings().stuckTimeout;
    }

    void SampleWorld(AntSwarmSimulation &simulation, AntAgent &ant, Registry &registry, const TileMap &tileMap) {
        ...
        SetTarget(ant, {.valid = true,
                        .position = ant.position + direction * simulation.Settings().sensorDistanceMin,
                        .direction = direction,
                        .distance = simulation.Settings().sensorDistanceMin});
    }

    SampleResult GetBestObjectiveSample(AntSwarmSimulation &simulation, const AntAgent &ant, Registry &registry,
                                        const TileMap &tileMap, float fov, int sampleCount) {
        SampleResult best;
        ...
        for (int index = 0; index < sampleCount; index++) {
            ...
            if (fieldName == "food") {
                ...
                if (food.GetId() >= 0 && glm::length(GetEntityPosition(food) - sample.position) <=
                                             simulation.Settings().pickupRadius * 2.0f) {
    ...
    SampleResult GetBestFallbackSample(AntSwarmSimulation &simulation, const AntAgent &ant, const TileMap &tileMap,
                                       float fov, int sampleCount) {
        SampleResult best;
        for (int index = 0; index < sampleCount; index++) {
            SampleResult sample = GetSample(simulation, ant, tileMap, fov);
            if (!sample.valid) {
    ...
    SampleResult GetSample(AntSwarmSimulation &simulation, const AntAgent &ant, const TileMap &tileMap, float fov) {
        ...
        return {.valid = true, .position = position, .direction = direction, .distance = distance};
    }
    ...
    void EscapeBlockedAnt(AntSwarmSimulation &simulation, AntAgent &ant, Registry &registry, const TileMap &tileMap) {
        if (TrySetDirectHomeTarget(simulation, ant, registry, tileMap)) {
            return;
        }
    ...
    bool TrySetDirectHomeTarget(AntSwarmSimulation &simulation, AntAgent &ant, Registry &registry,
    ...
    void UpdateMarker(AntSwarmSimulation &simulation, AntAgent &ant, const TileMap &tileMap,
                      const ProjectConfig &projectConfig) {
        ...
        const double intensity =
            simulation.Settings().trailDepositAmount * std::pow(1.0 - std::min(GetFieldDecay(simulation, marker), 0.95),
                                                                simulation.Settings().markerDecayPower * ant.walkTime);
        DepositField(simulation, tileMap, projectConfig, marker, ant.position, intensity,
                     simulation.Settings().trailRadius);
    }

    void DegradeFocusedMarker(AntSwarmSimulation &simulation, const AntAgent &ant, const TileMap &tileMap,
                              const ProjectConfig &projectConfig) {
        DepositField(simulation, tileMap, projectConfig, GetMarkerFocus(ant), ant.position,
                     -simulation.Settings().blockedDegradeAmount, simulation.Settings().trailRadius * 2.0f);
    }

    void DepositField(AntSwarmSimulation &simulation, const TileMap &tileMap, const ProjectConfig &projectConfig,
                      const std::string &fieldName, glm::vec2 position, double amount, float radius) {
        FieldDepositCommand command;
        ...
        command.falloff = "linear";
        FieldGridSolver::ApplyDeposit(simulation.Fields()[fieldName], tileMap, projectConfig.simulation, command);
    }
    ...
    std::string GetMarkerFocus(const AntAgent &ant) const { return ant.state == AntState::ToFood ? "food" : "home"; }
    ...
    float GetObjectiveFov(const AntSwarmSimulation &simulation, const AntAgent &ant) const {
        ...
        return ant.role == AntRole::Follower ? simulation.Settings().followerFovRadians
    ...
    int GetSampleCount(const AntSwarmSimulation &simulation, const AntAgent &ant) const {
        ...
        return ant.role == AntRole::Follower ? simulation.Settings().followerSampleCount
    ...
    double GetFieldDecay(const AntSwarmSimulation &simulation, const std::string &fieldName) const {
        return fieldName == "food" ? simulation.Settings().foodFieldDecayPerSecond
    ...
    Entity FindNearestFood(Registry &registry, glm::vec2 position) const {
        ...
        ForEachProjectEntity<FoodSourceComponent, TransformComponent>(
            registry, [&](Entity entity, FoodSourceComponent &food, TransformComponent &transform) {
                (void)food;
                (void)transform;
                if (GetFoodAmount(entity) <= 0) {
                    return;
    ...
    Entity FindNearestColony(Registry &registry, glm::vec2 position) const {
        ...
        float bestDistance = std::numeric_limits<float>::max();
        ForEachProjectEntity<AntColonyComponent, TransformComponent>(
            registry, [&](Entity entity, AntColonyComponent &colony, TransformComponent &transform) {
                (void)colony;
                (void)transform;
                const float distance = glm::length(GetEntityPosition(entity) - position);
                if (distance < bestDistance) {
    ...
    float SampleField(const AntSwarmSimulation &simulation, const std::string &fieldName,
        ...
        const std::size_t index =
            static_cast<std::size_t>(row) * static_cast<std::size_t>(fieldGrid.cols) + static_cast<std::size_t>(col);
        return index < fieldGrid.values.size() ? static_cast<float>(fieldGrid.values[index]) : 0.0f;
    }

    float MarkerSamplingCoefficient(const AntSwarmSimulation &simulation, const TileMap &tileMap, glm::vec2 position) {
    ...
    bool CanOccupy(const AntSwarmSimulation &simulation, const TileMap &tileMap, glm::vec2 position) const {
        return GridRaycaster::CanOccupyTileMap(
            tileMap, position, simulation.Settings().agentRadius,
            [](TerrainType terrain) { return terrain != TerrainType::Land && terrain != TerrainType::Runway; });
    }

    bool CanTravel(const AntSwarmSimulation &simulation, const TileMap &tileMap, glm::vec2 from, glm::vec2 to) const {
        return GridRaycaster::CanTravelTileMap(
            tileMap, from, to, simulation.Settings().agentRadius,
            [](TerrainType terrain) { return terrain != TerrainType::Land && terrain != TerrainType::Runway; });
    }
    ...
    glm::vec2 RandomOpenDirection(AntSwarmSimulation &simulation, const TileMap &tileMap, glm::vec2 position) {
    ...
    static glm::vec2 Rotate(glm::vec2 direction, float angle) {
        ...
        return SafeNormalize(glm::vec2(direction.x * c - direction.y * s, direction.x * s + direction.y * c),
                             direction);
    }
