...
bool ProjectScaffolder::CreateStandardStructure(const std::filesystem::path &root,
    ...
    if(!Write(root/"Source/Runtime/README.md","# Editor integration\n\nThis folder connects the project to PipeFrame scene authoring, registration, loading and lifecycle dispatch. Put simulation logic in Source/World/Runtime, physics in Source/World/Physics and render layers in Source/World/Rendering. Keep the generated adapter thin.\n",error))return false;
    if(!Write(root/"Source/World/Physics/ProjectPhysicsWorld.h","#pragma once\n#include <PipeFrame/World/World.h>\n#include <PipeFrame/Render/WorldDebugView.h>\nnamespace "+id+" {\nclass ProjectPhysicsWorld final : public pipeframe::PhysicsWorld {\npublic:\n    void CollectDebug(pipeframe::WorldDebugDraw &draw) const {\n        // Submit actual body shapes: draw.Circle(position, radius), draw.Polygon(corners).\n        (void)draw;\n    }\n    // Own physics systems here; register ordered updates with AddStep in the constructor.\n    // Replace defaultPhysics(delta) below when this project needs a custom solver.\n    template<class Step> void Simulate(float delta, Step defaultPhysics) {\n        defaultPhysics(delta);\n        Update(delta);\n    }\n};\n}\n",error))return false;
    if(!Write(root/"Source/World/Runtime/ProjectRuntimeWorld.h","#pragma once\n#include <PipeFrame/World/World.h>\n#include <PipeFrame/Render/WorldDebugView.h>\nnamespace "+id+" {\nclass ProjectRuntimeWorld final : public pipeframe::RuntimeWorld {\npublic:\n    explicit ProjectRuntimeWorld(pipeframe::BehaviourScene &scene) : RuntimeWorld(scene) {}\n    // Own project systems here. AddPreSceneSystem runs before behaviours;\n    // AddSystem runs after behaviours. All use the editor's actual scene.\n};\n}\n",error))return false;
    if(!Write(root/"Source/World/Rendering/ProjectRenderingWorld.h","#pragma once\n#include <PipeFrame/World/World.h>\n#include <PipeFrame/Render/WorldDebugView.h>\nnamespace "+id+" {\nclass ProjectRenderingWorld final : public pipeframe::RenderingWorld {\npublic:\n    void CollectDebug(pipeframe::WorldDebugDraw &draw) const {\n        // Submit your rendered triangles: draw.Mesh(vertices).\n        (void)draw;\n    }\n    // Own RenderLayer subclasses here and register them with AddLayer.\n    // Texture resources come from SceneProjectRuntime::GetVisualAssets().\n    // Replace drawScene() for a specialized batched renderer, as Ant does.\n    template<class Draw> void DrawScene(pipeframe::Canvas canvas, Draw drawScene) {\n        drawScene();\n        Render(canvas);\n    }\n};\n}\n",error))return false;
    if(!Write(root/"Source/World/ProjectWorld.h","#pragma once\n#include \"World/Physics/ProjectPhysicsWorld.h\"\n#include \"World/Runtime/ProjectRuntimeWorld.h\"\n#include \"World/Rendering/ProjectRenderingWorld.h\"\nnamespace "+id+" {\nclass ProjectWorld final : public pipeframe::World {\npublic:\n    explicit ProjectWorld(pipeframe::BehaviourScene &scene)\n        : runtime(scene), rendering(std::make_shared<ProjectRenderingWorld>()) {\n        SetRenderingWorld(rendering);\n    }\n    ProjectRuntimeWorld runtime;\n    ProjectPhysicsWorld physics;\n    std::shared_ptr<ProjectRenderingWorld> rendering;\n};\n}\n",error))return false;
    if(!Write(root/"Config/ProjectSettings.pipeframe","PIPEFRAME_PROJECT_SETTINGS 1\nunits meters\nfixedStep 0.016666667\n",error)||
       ...
       !Write(root/"Source/Runtime"/(id+"Runtime.h"),"#pragma once\n#include <PipeFrame/Project/SceneProjectRuntime.h>\n#include \"GeneratedRegistration.h\"\n#include \"World/ProjectWorld.h\"\nnamespace "+id+" {\nclass Runtime final : public pipeframe::SceneProjectRuntime {\npublic:\n    Runtime() : SceneProjectRuntime(\""+projectName+"\"), world(GetRuntimeScene()) {\n        pipeframe_generated::Register(*this);\n    }\n    void CollectWorldDebug(pipeframe::WorldDebugDraw &draw, pipeframe::WorldDebugOptions options) override {\n        SceneProjectRuntime::CollectWorldDebug(draw, options);\n        if(options.physics)world.physics.CollectDebug(draw);\n        if(options.mesh)world.rendering->CollectDebug(draw);\n    }\n    void Render(RenderContext &context) override {\n        world.rendering->DrawScene(context.GetCanvas(), [&] { SceneProjectRuntime::Render(context); });\n    }\nprotected:\n    void UpdateRuntimeScene(pipeframe::BehaviourScene &, float delta) override {\n        world.runtime.Update(delta);\n    }\n    void UpdatePhysics(float delta, const PreviousTransforms &previous) override {\n        world.physics.Simulate(delta, [&](float step) { SceneProjectRuntime::UpdatePhysics(step, previous); });\n    }\nprivate:\n    ProjectWorld world;\n};\n}\n",error)||
