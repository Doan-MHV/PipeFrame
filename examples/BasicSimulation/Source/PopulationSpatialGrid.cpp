...
void PopulationSpatialGrid::Initialize(
    ...
    float safeCellSize = requestedCellSize;

    if (!std::isfinite(safeCellSize) ||
        safeCellSize <= 0.0f) {

        safeCellSize = 1.0f;
    }

    const float minimumDimensionSafeCellSize =
        std::max(
            newLocalSize.x /
                static_cast<float>(
    ...
    const std::size_t safeMaximumAgentCount =
        std::min(
            maximumAgentCount,
            static_cast<std::size_t>(
                std::numeric_limits<
    ...
    agentCellIndices.resize(
        safeMaximumAgentCount);

    agentIndices.resize(
        safeMaximumAgentCount);
}

void PopulationSpatialGrid::Clear() {
    localMinimum = {};
    localSize = {};
...
void PopulationSpatialGrid::Rebuild(
...
PopulationSpatialGrid::CellRange
PopulationSpatialGrid::GetCellsOverlapping(
...
PopulationSpatialGrid::GetAgentIndices(
    const int column,
    const int row) const {
...
int PopulationSpatialGrid::GetColumnCount() const {
    return columnCount;
}

int PopulationSpatialGrid::GetRowCount() const {
    return rowCount;
}

float PopulationSpatialGrid::GetCellSize() const {
    return cellSize;
}

std::uint32_t
PopulationSpatialGrid::GetCellIndex(
...
sf::Vector2i
PopulationSpatialGrid::GetCellCoordinates(
    const sf::Vector2f position) const {
