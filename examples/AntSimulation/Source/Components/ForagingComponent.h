...
[[nodiscard]]
constexpr std::string_view ToString(const ForagingState state) {
    switch (state) {
...
struct ForagingComponent {
    ...
    static auto Schema() {
        ...
        return ComponentSchema<ForagingComponent>("ant.foraging", "Foraging")
            .Required()
            .ReadOnly({.key = "target", .displayName = "Target", .kind = K::Vector2, .defaultValue = Vector2f{}},
                      &ForagingComponent::target)
            .ReadOnly(
                {.key = "distanceToTarget", .displayName = "Target Distance", .kind = K::Number, .defaultValue = 0.0},
                &ForagingComponent::distanceToTarget);
    }
