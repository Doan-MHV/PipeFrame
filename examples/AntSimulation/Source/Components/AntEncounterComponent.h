...
struct AntEncounterComponent {
    ...
    static auto Schema() {
        ...
        return ComponentSchema<AntEncounterComponent>("ant.encounter", "Encounter")
            .Required()
            .ReadOnly({.key = "enemyTimer", .displayName = "Enemy Timer", .kind = K::Number, .defaultValue = 0.0},
                      &AntEncounterComponent::enemyTimer);
    }
