...
inline const pipeframe::EntityRegistry &AntEntityTypes() {
    static const auto types=[] {
        ...
        registry.Register<ColonyEntity>({"ant.colony","ANT COLONY",ColonySettingsComponent::Schema().Describe().properties,{"ant.colony"}});
        registry.Register<SignalBeaconEntity>({SignalBeaconTypeId,"SIGNAL BEACON",SignalBeaconComponent::Schema().Describe().properties,{SignalBeaconTypeId}});
        ...
        registry.Register<pipeframe::ComponentEntity<pipeframe::Transform2DComponent,SimulationSettingsComponent>>(
            {SimulationSettingsTypeId,"SIMULATION SETTINGS",SimulationSettingsComponent::Schema().Describe().properties,{SimulationSettingsTypeId}});
        registry.Register<pipeframe::ComponentEntity<pipeframe::Transform2DComponent,pipeframe::PlaygroundComponent,pipeframe::TilemapComponent>>(
            {pipeframe::PlaygroundEntityTypeId,"PLAYGROUND",{}, {pipeframe::PlaygroundComponentTypeId,pipeframe::TilemapComponentTypeId}});
        return registry;
    }();
    return types;
}
