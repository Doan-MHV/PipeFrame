...
class AntEntity final : public pipeframe::EntityArchetype {
  public:
    AntEntity(ColonyId colony, AntRole role, pipeframe::Vector2f position, float angle, float markerOffset,
              const AntConfiguration &configuration)
        : colony(colony), role(role), position(position), angle(angle), markerOffset(markerOffset),
