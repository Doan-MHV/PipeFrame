...
pipeframe::Vector2f GetCellCenter(
...
void ResetResult(
    ...
    const pipeframe::Vector2u mapSize = {0, 0}
) {
    result = {};
...
bool WorldMapLoader::LoadMapFromFile(
    ...
    if(!map){ResetResult(result);return false;}
...
bool WorldMapLoader::LoadMap(AntEnvironment &environment,const pipeframe::Tilemap2D &map,
    const std::filesystem::path &sourcePath,WorldMapLoadResult &result,std::string &errorMessage) {
    errorMessage.clear();
    ...
    if(map.CellSize()!=1 || map.Origin()!=pipeframe::Vector2f{}) {
        errorMessage="Ant currently requires unit tile cells and origin (0,0).";return false;
    }
    if (mapSize.x <=
            static_cast<unsigned int>(
                AntEnvironment::BorderMargin * 2) ||
        mapSize.y <=
            static_cast<unsigned int>(
                AntEnvironment::BorderMargin * 2)) {
        errorMessage =
            "Tilemap is too small to contain the "
    ...
    if(const auto *density=map.DataLayer("ant.food-density")){
        for(int y=0;y<map.Rows();++y)for(int x=0;x<map.Columns();++x){
            ...
            if(value<0||value>10000||std::floor(value)!=value){errorMessage="Ant food density must be an integer from 0 to 10000.";return false;}
        }
    }
    ...
    configuration.mapFilename =
        sourcePath.string();

    if (configuration.colonyPosition.x <
            static_cast<float>(
    ...
    for (unsigned int y = 0;
        ...
        for (unsigned int x = 0;
            ...
            const auto quantity=density?static_cast<std::size_t>(density->cells.At(coordinate)):
                0;
            if (quantity>0) {
