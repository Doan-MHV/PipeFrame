...
GridRaycastResult GridRaycast::Cast(const AntEnvironment &environment,
    ...
    if (!environment.IsInitialized() || maximumDistance<=0.0f ||
        !std::isfinite(start.x) || !std::isfinite(start.y)) return result;

    const auto hit=pipeframe::RaycastGrid(
        start,direction,maximumDistance,
        {0.0f,0.0f}, 1.0f, environment.GetWidth(), environment.GetHeight(),
        [&environment](const pipeframe::GridCoordinate coordinate) {
