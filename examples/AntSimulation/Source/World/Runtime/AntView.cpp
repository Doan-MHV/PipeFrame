...
void AntView::Initialize(
    const AntId newId,
    const ColonyId newColonyId,
    const AntRole newRole,
    const pipeframe::Vector2f initialPosition,
    ...
    const float safeMarkerOffset =
        std::max(
            0.0f,
            initialMarkerOffset);

    foraging.lastMarkerPosition =
        Transform().position -
        Pose().direction.GetTarget() *
            safeMarkerOffset;
}
...
void AntView::UpdateLegs(
    const float deltaTime
) {
    StepAntLegs(Pose(), Transform(), deltaTime);
}

void AntView::SetPosition(
    const pipeframe::Vector2f newPosition
) {
    Transform().position = newPosition;
}

void AntView::SetDirection(
    const pipeframe::Vector2f newDirection
) {
    const pipeframe::Vector2f normalized =
...
void AntView::SetTarget(
    const pipeframe::Vector2f newTarget
) {
    SetTarget(
        newTarget,
        Distance(Transform().position, newTarget));
}
...
void AntView::SetTarget(
    ...
    foraging.distanceToTarget =
        std::max(
            0.0f,
            distance);

    SetDirection(
        foraging.target - Transform().position);
}

void AntView::SetState(
    const ForagingState newState
) {
    auto &foraging = GetForagingComponent();
...
bool AntView::IsBlocked(
    const AntConfiguration &configuration
) const {
    auto &foraging = GetForagingComponent();
    return foraging.timeSinceLastMarker >
           MarkerTimeoutCoefficient *
               configuration
                   .GetAntMarkerInterval();
}

float AntView::GetBlockedRatio(
    const AntConfiguration &configuration
) const {
    auto &foraging = GetForagingComponent();
    const float markerInterval =
        configuration.GetAntMarkerInterval();

    const float denominator =
        (MarkerTimeoutCoefficient - 1.0f) *
        markerInterval;
...
float AntView::GetMarkerIntensity(
    const AntConfiguration &configuration
) const {
    auto &foraging = GetForagingComponent();
    return configuration.markerMaxIntensity *
           std::pow(
               ...
               4.0f * foraging.walkTime);
}

float AntView::GetEnemyMarkerIntensity(
    const AntConfiguration &configuration
) const {
    ...
    return configuration.markerMaxIntensity *
           std::pow(
               1.0f -
                   configuration.markerDecayRate,
               4.0f * GetEncounterComponent().enemyTimer);
}

bool AntView::IsDead() const {
    return GetEnergyComponent().IsDepleted();
}

void AntView::Kill() {
    GetEnergyComponent().Deplete();
}

void AntView::ConsumeEnergy(
    const float amount
) {
    if (amount <= 0.0f) {
        return;
...
float AntView::GetMass() const {
    return IsCarryingFood()
               ? FoodMass
               : BaseMass;
}

void AntView::BeginEncounter(
    const AntId opponent
) {
...
bool AntView::IsInEncounter() const {
    return GetEncounterComponent().opponentId.has_value();
}

float AntView::GetAngle() const {
    return Pose().direction.GetAngle();
}

pipeframe::Vector2f AntView::GetDirection() const {
    return Pose().direction.GetDirection();
}

AntId AntView::GetId() const {
    return ComponentView::GetEntity();
}

ColonyId AntView::GetColonyId() const {
    return Identity().colonyId;
}

AntRole AntView::GetRole() const {
...
pipeframe::Vector2f AntView::GetPosition() const {
    return Transform().position;
}

pipeframe::Vector2f AntView::GetVelocity() const {
    return GetDirection() * Motion().speed;
}
...
float AntView::GetEnergy() const {
    return GetEnergyComponent().current;
}

const std::array<AntLegPose, 6> &
AntView::GetLegs() const {
    return Pose().legs;
}

std::array<AntLegPose, 6> &
AntView::GetLegs() {
    return Pose().legs;
}

void AntView::CreateLegs() {
    InitializeAntLegs(Pose(), Transform());
}

pipeframe::Vector2f AntView::Normalize(
