...
class WallRenderer final : public pipeframe::RenderLayer {
  public:
    ...
    void Rebuild(const AntEnvironment &environment, const pipeframe::Rectanglef &viewport);
    ...
    [[nodiscard]]
    std::span<const pipeframe::Vertex2D> GetWallVertices() const;

    [[nodiscard]]
    std::span<const pipeframe::Vertex2D> GetShadowVertices() const;

  private:
    void AddWall(pipeframe::Vector2f position, WallType type);

    static void AddShape(std::vector<pipeframe::Vertex2D> &vertices, pipeframe::Vector2f position, WallType type,
                         pipeframe::Color color, pipeframe::Vector2f offset);
