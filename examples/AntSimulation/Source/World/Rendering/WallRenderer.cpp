...
void WallRenderer::SetShadowEnabled(
    const bool enabled
) {
...
void WallRenderer::Rebuild(
    ...
    wallVertices.reserve(
        candidateCount * 6);

    if (shadowEnabled) {
        shadowVertices.reserve(
            candidateCount * 6);
    }
...
void WallRenderer::Draw(
    ...
    if (shadowEnabled &&
        ...
        pipeframe::RenderState shadowStates =
            states;

        shadowStates.blendMode =
            pipeframe::BlendMode::Alpha;
...
bool WallRenderer::IsShadowEnabled() const {
    return shadowEnabled;
}

std::size_t
WallRenderer::GetCandidateCount() const {
    return candidateCount;
}

std::size_t
WallRenderer::GetVisibleWallCount() const {
    return visibleWallCount;
}

std::span<const pipeframe::Vertex2D>
WallRenderer::GetWallVertices() const {
    return wallVertices;
}

std::span<const pipeframe::Vertex2D>
WallRenderer::GetShadowVertices() const {
    return shadowVertices;
}

void WallRenderer::AddWall(
    const pipeframe::Vector2f position,
    const WallType type
) {
    AddShape(
...
void WallRenderer::AddShape(
    std::vector<pipeframe::Vertex2D> &vertices,
    const pipeframe::Vector2f position,
    const WallType type,
    const pipeframe::Color color,
    const pipeframe::Vector2f offset
) {
    const pipeframe::Vector2f p =
        position +
        offset;
    ...
    std::array<pipeframe::Vector2f, 6>
        positions{};
    ...
    for (const pipeframe::Vector2f vertexPosition :
         positions) {
...
bool WallRenderer::IsVisible(
    const pipeframe::Vector2f position,
    const pipeframe::Rectanglef &viewport
) {
    const float cellMaximumX =
        position.x + 1.0f;

    const float cellMaximumY =
        position.y + 1.0f;

    const float viewportMaximumX =
        viewport.position.x +
        viewport.size.x;

    const float viewportMaximumY =
        viewport.position.y +
        viewport.size.y;
