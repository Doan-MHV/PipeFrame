...
void AntDebugRenderer::SetSelectedAnt(
    const std::optional<AntId> antId
) {
    selectedAntId = antId;
...
void AntDebugRenderer::SetTargetVisible(
    const bool visible
) {
...
void AntDebugRenderer::SetPhysicsDebugVisible(
    const bool visible
) {
...
void AntDebugRenderer::Update(
    const std::span<const AntView> ants,
    const std::span<const AntPhysicsBody> bodies,
    ...
    if (selectedAntId.has_value()) {
        ...
        if (iterator != ants.end() &&
            !iterator->IsDead()) {
            const AntView &selectedAnt =
                *iterator;

            if (IsVisible(
                    selectedAnt.GetPosition(),
                    SelectionRadius,
                    viewport)) {
                AddRing(
                    selectionVertices,
    ...
    physicsVertices.reserve(
        bodies.size() *
        PhysicsCircleSegmentCount *
        3);

    pipeframe::VisitBodyDebugGeometry<AntPhysicsBody>(bodies,
        [](const AntPhysicsBody &body){return body.moving?MovingPhysicsColor:StaticPhysicsColor;},
...
std::optional<AntId>
AntDebugRenderer::GetSelectedAnt() const {
    return selectedAntId;
}

bool AntDebugRenderer::IsTargetVisible() const {
    return targetVisible;
}

bool AntDebugRenderer::IsPhysicsDebugVisible() const {
    return physicsDebugVisible;
}

std::size_t
AntDebugRenderer::GetPhysicsCandidateCount() const {
    return physicsCandidateCount;
}

std::size_t
AntDebugRenderer::GetVisiblePhysicsBodyCount() const {
    return visiblePhysicsBodyCount;
}

std::span<const pipeframe::Vertex2D>
AntDebugRenderer::GetSelectionVertices() const {
    return selectionVertices;
}

std::span<const pipeframe::Vertex2D>
AntDebugRenderer::GetTargetVertices() const {
    return targetVertices;
}

std::span<const pipeframe::Vertex2D>
AntDebugRenderer::GetPhysicsVertices() const {
    return physicsVertices;
}

void AntDebugRenderer::AddCircle(
...
void AntDebugRenderer::AddRing(
    ...
    for (std::size_t segment = 0;
        ...
        const std::array<pipeframe::Vector2f, 6>
            positions{
                outerFirst,
                outerSecond,
                innerFirst,
        ...
        for (const pipeframe::Vector2f position :
             positions) {
...
bool AntDebugRenderer::IsVisible(
    const pipeframe::Vector2f center,
    const float radius,
    const pipeframe::Rectanglef &viewport
) {
