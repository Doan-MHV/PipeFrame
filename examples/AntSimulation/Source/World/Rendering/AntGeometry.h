...
class AntGeometry {
  public:
    ...
    void UpdateDetailed(const AntView &ant, std::size_t antIndex, const AntConfiguration &configuration);

    void UpdateSimple(const AntView &ant, std::size_t antIndex, const AntConfiguration &configuration);
    ...
    [[nodiscard]]
    static pipeframe::Color GetAntColor(const AntView &ant, const AntConfiguration &configuration);

  private:
    static void WriteQuad(std::vector<pipeframe::Vertex2D> &vertices, std::size_t quadIndex, pipeframe::Vector2f center,
                          pipeframe::Vector2f direction, pipeframe::Vector2f size, pipeframe::Vector2f textureMinimum,
                          pipeframe::Vector2f textureMaximum, pipeframe::Color color);

    static void ClearQuad(std::vector<pipeframe::Vertex2D> &vertices, std::size_t quadIndex);

    void UpdateFood(const AntView &ant, std::size_t antIndex, const AntConfiguration &configuration);

    static pipeframe::Vector2f Normalize(pipeframe::Vector2f value);
