...
struct AntFixtureScene { pipeframe::BehaviourScene scene; };
class AntFixture : private AntFixtureScene, public AntView {
    static pipeframe::SceneObject Create(pipeframe::BehaviourScene &scene, AntId id,
        ColonyId colony, AntRole role, pipeframe::Vector2f position, float angle,
        float markerOffset, const AntConfiguration &configuration) {
...
public:
    AntFixture(AntId id, ColonyId colony, AntRole role, pipeframe::Vector2f position,
