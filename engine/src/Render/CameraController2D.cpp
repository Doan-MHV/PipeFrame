...
void CameraController2D::CancelInteraction() {
    dragButton.reset();
    spacePressed = false;
}
...
void CameraController2D::HandleEvent(const pipeframe::InputEvent &event, RenderContext &context) {
    ...
    if (event.type == InputEventType::FocusChanged) {
        if (const auto *focus = event.GetIf<FocusInput>(); focus && !focus->focused)
            CancelInteraction();
    }
    ...
    if (event.type == InputEventType::PointerPressed) {
        if (const auto *pressed = event.GetIf<PointerInput>();
            pressed &&
    ...
    if (event.type == InputEventType::PointerReleased) {
        if (const auto *released = event.GetIf<PointerInput>(); released && dragButton == released->button)
            dragButton.reset();
    }
    if (event.type == InputEventType::PointerMoved) {
        if (const auto *moved = event.GetIf<PointerMoveInput>()) {
            if (dragButton)
                context.GetCamera().Move(context.MapPixelToWorld(previousMousePosition) -
                                         context.MapPixelToWorld(moved->position));
