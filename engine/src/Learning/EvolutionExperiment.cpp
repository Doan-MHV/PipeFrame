...
bool EvolutionExperiment::Initialize(EvolutionExperimentConfig newConfiguration, std::string &errorMessage) {
    Clear();
    if (newConfiguration.inputCount == 0 || newConfiguration.outputCount == 0 ||
        newConfiguration.population == 0 || newConfiguration.population > 1'000'000 ||
        !std::isfinite(newConfiguration.eliteRatio) || newConfiguration.eliteRatio <= 0.0f ||
        newConfiguration.eliteRatio > 1.0f) {
        errorMessage = "Invalid evolution experiment configuration.";
        return false;
    }
    configuration = newConfiguration;
    if (!CreateInitialPopulation(errorMessage)) return false;
    initialized = true;
    errorMessage.clear();
    return true;
}
...
bool EvolutionExperiment::RunGeneration(const GenomeEvaluator &evaluator, std::string &errorMessage) {
    if (!initialized || !evaluator) { errorMessage = "Evolution experiment is not initialized."; return false; }
    ...
    for (std::size_t index = 0; index < population.size(); ++index) {
        ...
        total += score;
        if (score > generationBest) { generationBest = score; bestIndex = index; }
    }
    averageScore = total / static_cast<float>(scores.size());
    if (!bestGenome || generationBest >= bestScore) { bestGenome = population[bestIndex]; bestScore = generationBest; }
    history.Add({run, generation, generationBest, averageScore, 1.0f,
                 population[bestIndex].GetNodeCount(), population[bestIndex].GetConnections().size(), {}});
    if (!Evolve(scores, errorMessage)) return false;
    ++generation;
    errorMessage.clear();
    return true;
}
...
void EvolutionExperiment::Clear() {
    population.clear(); history.Clear(); bestGenome.reset(); run = 0; generation = 0;
    bestScore = 0.0f; averageScore = 0.0f; initialized = false;
}

bool EvolutionExperiment::SaveCheckpoint(const std::filesystem::path &directory, std::string &errorMessage) const {
    if (!initialized) { errorMessage = "Evolution experiment is not initialized."; return false; }
    std::error_code filesystemError;
    std::filesystem::create_directories(directory / "Population", filesystemError);
    if (filesystemError) { errorMessage = "Unable to create checkpoint: " + filesystemError.message(); return false; }
    CheckpointMetadata metadata{"pipeframe.neat", 1, run, generation, configuration.seed, population.size(), {
        {"inputs", std::to_string(configuration.inputCount)}, {"outputs", std::to_string(configuration.outputCount)},
        {"elite_ratio", std::to_string(configuration.eliteRatio)}, {"best_score", std::to_string(bestScore)},
    ...
    if (!metadata.Save(directory / "checkpoint.pftrain", errorMessage)) return false;
    for (std::size_t index = 0; index < population.size(); ++index) {
        if (!population[index].Save(directory / "Population" / ("agent_" + std::to_string(index) + ".pfneat"), errorMessage)) return false;
    }
    if (!history.SaveCsv(directory / "history.csv", errorMessage)) return false;
...
bool EvolutionExperiment::LoadCheckpoint(const std::filesystem::path &directory, std::string &errorMessage) {
    ...
    if (!metadata.Load(directory / "checkpoint.pftrain", errorMessage) || metadata.trainerId != "pipeframe.neat" ||
        ...
        if (errorMessage.empty()) errorMessage = "Checkpoint is not a PipeFrame NEAT experiment.";
        return false;
    }
    ...
    try {
        ...
        loadedConfig.mutation.mutationCount = static_cast<std::uint32_t>(std::stoul(metadata.values.at("mutation_count")));
        loadedConfig.mutation.maximumHiddenNodes = static_cast<std::uint32_t>(std::stoul(metadata.values.at("maximum_hidden_nodes")));
        loadedConfig.population = metadata.population;
        loadedConfig.seed = metadata.seed;
    } catch (...) { errorMessage = "Checkpoint evolution configuration is invalid."; return false; }
    std::vector<Genome> loadedPopulation(metadata.population);
    for (std::size_t index = 0; index < loadedPopulation.size(); ++index) {
        if (!loadedPopulation[index].Load(directory / "Population" / ("agent_" + std::to_string(index) + ".pfneat"), errorMessage)) return false;
    }
    ExperimentHistory loadedHistory;
    if (!loadedHistory.LoadCsv(directory / "history.csv", errorMessage)) return false;
    std::optional<Genome> loadedBest;
    if (std::filesystem::exists(directory / "best.pfneat")) {
        Genome value;
        if (!value.Load(directory / "best.pfneat", errorMessage)) return false;
        loadedBest = std::move(value);
    }
    configuration = loadedConfig; population = std::move(loadedPopulation); history = std::move(loadedHistory);
    bestGenome = std::move(loadedBest); run = metadata.run; generation = metadata.iteration;
    try { bestScore = std::stof(metadata.values.at("best_score")); } catch (...) { bestScore = 0.0f; }
    initialized = true; errorMessage.clear(); return true;
}
...
std::optional<InferenceSnapshot> EvolutionExperiment::BestInferenceSnapshot(std::string *errorMessage) const {
    if (!bestGenome) { if (errorMessage) *errorMessage = "No evaluated genome is available."; return std::nullopt; }
...
bool EvolutionExperiment::CreateInitialPopulation(std::string &errorMessage) {
    population.clear(); population.reserve(configuration.population);
    ...
    for (std::size_t index = 0; index < configuration.population; ++index) {
        ...
        if (index > 0 && !mutator.AddConnection(genome)) { errorMessage = "Unable to seed evolution population."; return false; }
        if (index > 0) mutator.Mutate(genome);
...
bool EvolutionExperiment::Evolve(const std::span<const float> scores, std::string &errorMessage) {
    ...
    for (std::size_t index = 0; index < eliteCount; ++index) {
        ...
        if (!signatures.insert(genome.GetSignature()).second) mutator.Mutate(genome);
        next.push_back(std::move(genome));
    }
    while (next.size() < population.size()) {
        Genome genome = population[selector.Pick()]; mutator.Mutate(genome); next.push_back(std::move(genome));
    }
    population = std::move(next); errorMessage.clear(); return true;
}
