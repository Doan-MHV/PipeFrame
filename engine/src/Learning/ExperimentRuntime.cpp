...
void ExperimentClock::Advance(const float deltaTime) {
    if (std::isfinite(deltaTime) && deltaTime > 0.0f) elapsed += deltaTime;
}
...
bool ExperimentHistory::SaveCsv(const std::filesystem::path &path, std::string &errorMessage) const {
    ...
    if (!output) { errorMessage = "Unable to create experiment history: " + path.string(); return false; }
    ...
    if (!output.good()) { errorMessage = "Unable to write experiment history: " + path.string(); return false; }
    return true;
}
...
bool ExperimentHistory::LoadCsv(const std::filesystem::path &path, std::string &errorMessage) {
    ...
    if (!input) { errorMessage = "Unable to open experiment history: " + path.string(); return false; }
    std::string line;
    if (!std::getline(input, line)) { errorMessage = "Experiment history is empty."; return false; }
    ...
    while (std::getline(input, line)) {
        ...
        char separator{};
        if (!(row >> sample.run >> separator) || separator != ',' ||
            !(row >> sample.iteration >> separator) || separator != ',' ||
            !(row >> sample.bestScore >> separator) || separator != ',' ||
            !(row >> sample.averageScore >> separator) || separator != ',' ||
            !(row >> sample.progress >> separator) || separator != ',' ||
            !(row >> sample.modelNodeCount >> separator) || separator != ',' ||
            !(row >> sample.modelConnectionCount >> separator) || separator != ',') {
            errorMessage = "Invalid experiment history row: " + line; return false;
        }
        std::string encoded;
        ...
        while (std::getline(metrics, item, ';')) {
            ...
            try { sample.metrics[item.substr(0, equals)] = std::stod(item.substr(equals + 1)); }
            catch (...) { errorMessage = "Invalid experiment metric: " + item; return false; }
        }
...
bool CheckpointMetadata::Save(const std::filesystem::path &path, std::string &errorMessage) const {
    errorMessage.clear();
    std::ofstream output(path, std::ios::trunc);
    if (!output) { errorMessage = "Unable to create checkpoint metadata: " + path.string(); return false; }
    output << "PIPEFRAME_EXPERIMENT " << version << '\n' << trainerId << '\n'
           << run << ' ' << iteration << ' ' << seed << ' ' << population << '\n';
    for (const auto &[key, value] : values) output << key << '=' << value << '\n';
    if (!output.good()) { errorMessage = "Unable to write checkpoint metadata: " + path.string(); return false; }
    return true;
}
...
bool CheckpointMetadata::Load(const std::filesystem::path &path, std::string &errorMessage) {
    ...
    if (!(input >> magic >> loaded.version) || magic != "PIPEFRAME_EXPERIMENT" || loaded.version != 1) {
        errorMessage = "Invalid or unsupported experiment checkpoint metadata."; return false;
    }
    input.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
    if (!std::getline(input, loaded.trainerId) ||
        !(input >> loaded.run >> loaded.iteration >> loaded.seed >> loaded.population)) {
        errorMessage = "Incomplete experiment checkpoint metadata."; return false;
    }
    input.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
    std::string line;
    while (std::getline(input, line)) {
        const std::size_t equals = line.find('=');
        if (equals != std::string::npos) loaded.values[line.substr(0, equals)] = line.substr(equals + 1);
    }
