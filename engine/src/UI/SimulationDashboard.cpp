...
const sf::Font &CheckedFont(const pipeframe::GraphicsResourceService &resources, pipeframe::FontHandle handle) {
    ...
    if (!font)
        throw std::invalid_argument("Dashboard requires a loaded font handle");
    return *font;
}
UITheme NativeTheme(const pipeframe::ui::ViewTheme &value) {
    UITheme result;
    result.applicationBackground = {value.applicationBackground.r, value.applicationBackground.g,
                                    value.applicationBackground.b, value.applicationBackground.a};
    result.surface = {value.surface.r, value.surface.g, value.surface.b, value.surface.a};
    result.elevatedSurface = {value.elevatedSurface.r, value.elevatedSurface.g, value.elevatedSurface.b,
                              value.elevatedSurface.a};
    result.inputBackground = {value.inputBackground.r, value.inputBackground.g, value.inputBackground.b,
                              value.inputBackground.a};
    result.floatingSurface = {value.floatingSurface.r, value.floatingSurface.g, value.floatingSurface.b,
                              value.floatingSurface.a};
    ...
    result.controlHovered = {value.controlHovered.r, value.controlHovered.g, value.controlHovered.b,
                             value.controlHovered.a};
    result.controlPressed = {value.controlPressed.r, value.controlPressed.g, value.controlPressed.b,
                             value.controlPressed.a};
    result.controlDisabled = {value.controlDisabled.r, value.controlDisabled.g, value.controlDisabled.b,
                              value.controlDisabled.a};
    result.controlSelected = {value.controlSelected.r, value.controlSelected.g, value.controlSelected.b,
                              value.controlSelected.a};
...
struct SimulationDashboard::Impl {
    ...
    Impl(const pipeframe::GraphicsResourceService &resources, pipeframe::FontHandle font, const std::string &title,
         const pipeframe::ui::ViewTheme &theme)
...
SimulationDashboard::SimulationDashboard(const pipeframe::GraphicsResourceService &resources,
                                         pipeframe::FontHandle font, const std::string &title,
                                         const pipeframe::ui::ViewTheme &theme)
...
void SimulationDashboard::AddViewDrawer(DrawerEdge edge, const std::string &title, float width, float height,
                                        DrawerAnchor anchor, std::function<pipeframe::ui::View()> build) {
    impl->host.AddViewDrawer(edge, title, width, height, anchor, std::move(build));
}
void SimulationDashboard::AddViewMetric(float width, float height, std::function<pipeframe::ui::View()> build) {
    impl->host.AddViewMetric(width, height, std::move(build));
}
...
void SimulationDashboard::SetDrawerOpen(std::size_t index, bool open, bool animate) {
    impl->host.GetIndependentDrawer(index).SetOpen(open, animate);
}
bool SimulationDashboard::IsDrawerOpen(std::size_t index) const {
    return impl->host.GetIndependentDrawer(index).IsOpen();
}
...
NativeSimulationDashboard &pipeframe::backend::sfml::DashboardAccess::Host(SimulationDashboard &dashboard) {
    return dashboard.impl->host;
}
