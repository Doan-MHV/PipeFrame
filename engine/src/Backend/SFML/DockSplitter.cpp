...
void DockSplitter::SetAxis(Axis axis) { controller.SetAxis(axis); RefreshColor(); }
...
bool DockSplitter::OnEvent(const sf::Event &event) {
    ...
    if (event.is<sf::Event::FocusLost>() || event.is<sf::Event::MouseLeft>()) hovered = false;
    RefreshColor();
    return handled;
}
void DockSplitter::OnPointerEntered() { hovered = true; RefreshColor(); }
void DockSplitter::OnPointerExited() { hovered = false; RefreshColor(); }
