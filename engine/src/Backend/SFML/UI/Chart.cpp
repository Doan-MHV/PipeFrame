...
ChartRange Resolve(ChartRange range) {
    if (range.minimum > range.maximum) return {0.0, 1.0};
...
bool Clip(double &x0, double &y0, double &x1, double &y1, ChartRange x, ChartRange y) {
    ...
    const auto edge = [&](const double p, const double q) {
        ...
        if (p < 0.0) enter = std::max(enter, ratio);
        else leave = std::min(leave, ratio);
        return enter <= leave;
    };
    if (!edge(-dx, x0 - x.minimum) || !edge(dx, x.maximum - x0) ||
        !edge(-dy, y0 - y.minimum) || !edge(dy, y.maximum - y0)) return false;
...
void Chart::SetHorizontalRange(const std::optional<ChartRange> range) {
    Validate(range); horizontalRange = range; RebuildGeometry();
}
void Chart::SetVerticalRange(const std::optional<ChartRange> range) {
    Validate(range); verticalRange = range; RebuildGeometry();
}
...
void Chart::SetGridColor(const sf::Color color) { gridColor = color; RebuildGeometry(); }
void Chart::SetGridDivisions(const unsigned int divisions) {
    gridDivisions = std::min(divisions, 32u); RebuildGeometry();
}
...
void Chart::OnRender(sf::RenderTarget &target) const {
    if (!gridVertices.empty()) target.draw(gridVertices.data(), gridVertices.size(), sf::PrimitiveType::Lines);
    if (!plotVertices.empty()) target.draw(plotVertices.data(), plotVertices.size(),
...
void Chart::RebuildGeometry() {
    ...
    for (const auto &entry : series) for (const auto point : entry.samples) {
    ...
    gridVertices.clear(); plotVertices.clear();
    ...
    if (gridDivisions > 0) for (unsigned int i = 0; i <= gridDivisions; ++i) {
    ...
    for (const auto &entry : series) {
        ...
        for (std::size_t i = 0; i < entry.samples.size(); ++i) {
            ...
            if (!Finite(point)) continue;
            if (kind == Kind::Line) {
                if (i == 0 || !Finite(entry.samples[i - 1])) continue;
...
void BarChart::SetValues(const std::vector<float> &values, const sf::Color color) {
    ...
    for (std::size_t i = 0; i < values.size(); ++i) entry.samples.emplace_back(static_cast<float>(i), values[i]);
