...
TabView::TabView() : tabs(CreateChild<SegmentedControl>()), pages(CreateChild<OverlayPanel>()) {
    ...
    tabs.SetOnSelectionChanged([this](const std::size_t index) { ApplySelection(index, true); });
}
...
Widget *TabView::GetPage(const std::size_t index) { return index < pageWidgets.size() ? pageWidgets[index] : nullptr; }
...
void TabView::SetOnSelectionChanged(SelectionChangedCallback callback) { onSelectionChanged = std::move(callback); }
