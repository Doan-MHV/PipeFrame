...
NetworkView::NetworkView(const UITheme &newTheme) : selectionColor(newTheme.textPrimary), theme(newTheme) {
    SetHitTestVisible(false);
    SetSize({240, 160});
}
NetworkView::NetworkView(const sf::Font &newFont, const UITheme &newTheme) : NetworkView(newTheme) { font = &newFont; }
...
void NetworkView::SetGraph(std::vector<NetworkNode> newNodes, std::vector<NetworkEdge> newEdges) {
    ...
    for (std::size_t i = 0; i < newNodes.size(); ++i) {
        ...
        if (!std::isfinite(node.position.x) || !std::isfinite(node.position.y) || node.position.x < 0 ||
            node.position.x > 1 || node.position.y < 0 || node.position.y > 1 || !ids.emplace(node.id, i).second) {
...
void NetworkView::SetNodeRadius(const float value) {
    if (!std::isfinite(value) || value <= 0)
        throw std::invalid_argument("Node radius must be finite and positive");
    radius = value;
    RebuildGeometry();
}
...
std::optional<std::uint64_t> NetworkView::FindNodeAt(const sf::Vector2f point) const {
    ...
    for (std::size_t i = centers.size(); i > 0; --i) {
        ...
        if (delta.x * delta.x + delta.y * delta.y <= visualRadius * visualRadius)
            return nodes[i - 1].id;
    }
    return std::nullopt;
}
...
void NetworkView::RebuildGeometry() {
    ...
    for (const auto &edge : edges) {
        ...
        triangle(start + normal * halfWidth, start - normal * halfWidth, base + normal * halfWidth, color);
        triangle(base + normal * halfWidth, start - normal * halfWidth, base - normal * halfWidth, color);
        if (arrow > 0)
            triangle(end, base + normal * (arrow * 0.5f), base - normal * (arrow * 0.5f), color);
    }
    ...
    for (std::size_t i = 0; i < nodes.size(); ++i) {
        if (selected == nodes[i].id)
            disc(centers[i], visualRadius + 3, selectionColor);
