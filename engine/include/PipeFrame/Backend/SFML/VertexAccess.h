...
inline void ConvertVertices(std::span<const Vertex2D> source, std::vector<sf::Vertex>& result) {
    ...
    for (std::size_t i = 0; i < source.size(); ++i)
        result[i] = {ToBackend(source[i].position), ToBackend(source[i].color), ToBackend(source[i].textureCoordinate)};
}

inline void DrawVertices(sf::RenderTarget& target, std::span<const Vertex2D> source, sf::PrimitiveType primitive,
