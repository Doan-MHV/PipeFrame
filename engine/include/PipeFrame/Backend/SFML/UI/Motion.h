...
inline sf::Color LerpColor(const sf::Color from, const sf::Color to, const float progress) {
    ...
    const auto channel = [eased](const std::uint8_t start, const std::uint8_t end) {
        return static_cast<std::uint8_t>(std::clamp(
            std::lround(static_cast<float>(start) +
                        (static_cast<float>(end) - static_cast<float>(start)) * eased),
            0l, 255l));
    };
...
class AnimatedFloat final {
  public:
    explicit AnimatedFloat(const float initial = 0.0f)
...
class AnimatedVector2 final {
  public:
    explicit AnimatedVector2(const sf::Vector2f initial = {0.0f, 0.0f})
