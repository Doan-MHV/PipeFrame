...
class Widget {
  public:
    ...
    template<class T, class... Args> T &ReconcileChild(const std::string &key, Args&&... args) {
    ...
    template <typename Handle> void RetainBinding(Handle &&handle) {
        retainedBindings.push_back(
            std::make_shared<std::decay_t<Handle>>(std::forward<Handle>(handle)));
    }
