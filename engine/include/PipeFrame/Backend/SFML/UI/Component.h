...
auto Component(std::string key, ConfigureFunction &&configure, ChildSpecs &&...children) {
    ...
    return ComponentSpec<WidgetType, std::decay_t<ConfigureFunction>, std::decay_t<ChildSpecs>...>{
        std::move(key), std::forward<ConfigureFunction>(configure),
        std::forward<ChildSpecs>(children)...};
}

template <typename... ComponentSpecs> void Compose(Widget &parent, ComponentSpecs &&...components) {
