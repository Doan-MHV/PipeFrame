...
public:
    NativeSimulationDashboard(const pipeframe::GraphicsResourceService& resources, pipeframe::FontHandle font,
        ...
        : NativeSimulationDashboard(*pipeframe::backend::sfml::GraphicsResourceAccess::Font(resources, font), title,
                                    theme) {}
    void Layout(const RenderContext& context) {
        Layout(sf::FloatRect(pipeframe::backend::sfml::ViewportBounds(context)));
    }
    void Render(RenderContext& context) { Render(pipeframe::backend::sfml::GetTarget(context)); }
    bool ContainsPoint(pipeframe::Vector2i point) const {
    ...
    explicit NativeSimulationDashboard(const sf::Font& font, const std::string& title,
        ...
        drawer->SetOnOpenChanged([this](bool open) {
            ...
            if (!open) ui.HandleEvent(sf::Event::FocusLost{});
        });
    }
    ...
    void AddViewMetric(float width, float height, std::function<pipeframe::ui::View()> builder) {
        ...
        view.SetHitTestVisible(false);
        metricViews.push_back(&view);
    }

    void InvalidateViews() {
        for (auto& entry : viewPanels)
            entry.first->InvalidateView();
    ...
    MetricCard& AddFloatingMetric(const std::string& title, float width, float height, DrawerAnchor horizontal,
    ...
    Column& AddIndependentDrawer(DrawerEdge edge, const std::string& handleText, float extent = 300.0f,
                                 float span = 0.0f, DrawerAnchor anchor = DrawerAnchor::Center,
        ...
        independent.SetOnOpenChanged([this, &independent, &scroll, &content](const bool open) {
            ...
            if (open) {
                for (auto& entry : independentDrawers) {
                    if (entry.drawer == &independent || !entry.drawer->IsOpen()) continue;
                ...
                const bool anotherPopupOpen =
                    std::any_of(independentDrawers.begin(), independentDrawers.end(),
    ...
    void Layout(sf::FloatRect viewport) {
        ...
        const bool popupOpen =
            std::ranges::any_of(independentDrawers, [](const auto& entry) { return entry.drawer->IsOpen(); });
        for (auto* view : metricViews) {
        ...
        for (auto& entry : independentDrawers) {
            ...
            const bool vertical =
                entry.drawer->GetEdge() == DrawerEdge::Left || entry.drawer->GetEdge() == DrawerEdge::Right;
            ...
            const float primary =
                std::max(40.0f, std::min(scaledExtent, (vertical ? viewport.size.x : viewport.size.y) - 20.0f));
            const float availableSpan = vertical ? viewport.size.y : viewport.size.x;
            const float requestedSpan =
                entry.span > 0.0f ? entry.span * (vertical ? 1.0f : std::max(1.0f, uiScale)) : availableSpan;
            const float cross = std::max(40.0f, std::min(requestedSpan, availableSpan));
            const sf::Vector2f drawerSize = vertical ? sf::Vector2f{primary, cross} : sf::Vector2f{cross, primary};
            sf::Vector2f position{};
            if (vertical) {
                position.y = AnchorPosition(viewport.size.y, drawerSize.y, entry.anchor);
                if (entry.drawer->GetEdge() == DrawerEdge::Right) position.x = viewport.size.x - drawerSize.x;
            } else {
                position.x = AnchorPosition(viewport.size.x, drawerSize.x, entry.anchor);
                if (entry.drawer->GetEdge() == DrawerEdge::Bottom) position.y = viewport.size.y - drawerSize.y;
            }
            ...
            if (vertical) {
                const float center =
                    viewport.size.y * static_cast<float>(slot + 1) / static_cast<float>(edgeCounts[edgeIndex] + 1);
                handlePosition.y = center - position.y - handleSize.y * 0.5f;
            } else {
                const float center =
                    viewport.size.x * static_cast<float>(slot + 1) / static_cast<float>(edgeCounts[edgeIndex] + 1);
            ...
            for (auto& [view, index] : viewPanels)
                if (independentDrawers[index].drawer == entry.drawer)
...
private:
    ...
    static float AnchorPosition(float available, float occupied, DrawerAnchor anchor) {
        if (anchor == DrawerAnchor::Start) return 8.0f;
        if (anchor == DrawerAnchor::End) return std::max(0.0f, available - occupied - 8.0f);
