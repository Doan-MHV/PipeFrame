...
inline Canvas MakeCanvas(sf::RenderTarget& target) {
    return Canvas(&target, [](void* opaque, std::span<const Vertex2D> vertices, PrimitiveTopology topology,
                              const RenderState& state) {
        sf::RenderStates native;
        native.blendMode = state.blendMode == BlendMode::Add ? sf::BlendAdd : sf::BlendAlpha;
        if (state.texture.resources)
            native.texture = GraphicsResourceAccess::Texture(*state.texture.resources, state.texture.handle);
        if (state.shader.resources) {
            auto* shader = GraphicsResourceAccess::Shader(const_cast<GraphicsResourceService&>(*state.shader.resources),
                                                          state.shader.handle);
