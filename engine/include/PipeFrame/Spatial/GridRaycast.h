...
std::optional<GridRaycastHit> RaycastGrid(Vector2f origin, Vector2f direction, float maxDistance, Vector2f gridOrigin,
                                          float cellSize, int columns, int rows, IsBlocked isBlocked) {
    const float length = Length(direction);
    if (!std::isfinite(length) || length <= 0 || !std::isfinite(maxDistance) || maxDistance < 0 ||
        !std::isfinite(cellSize) || cellSize <= 0 || columns <= 0 || rows <= 0 || !std::isfinite(origin.x) ||
        !std::isfinite(origin.y) || !std::isfinite(gridOrigin.x) || !std::isfinite(gridOrigin.y))
        return std::nullopt;
    direction /= length;
    ...
    if (enter == 0 && ((x >= columns && direction.x >= 0) || (x < 0 && direction.x <= 0) ||
                       (y >= rows && direction.y >= 0) || (y < 0 && direction.y <= 0)))
        return std::nullopt;
