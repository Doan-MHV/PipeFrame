    ...
    return std::visit(
        [](const auto& value) -> std::string {
            ...
            if constexpr (std::is_same_v<T, bool>)
                ...
                out << int(value.r) << ", " << int(value.g) << ", " << int(value.b) << ", " << int(value.a);
            else if constexpr (std::is_same_v<T, AssetReference>)
                out << value.assetId;
            else if constexpr (std::is_same_v<T, SceneObjectReference>)
                out << value.objectId;
            else
                out << value;
            return out.str();
...
template <class T>
T ReadNumber(const std::string& text) {
    T result{};
...
inline PropertyValue ParseProperty(PropertyKind kind, const std::string& text) {
    switch (kind) {
        case PropertyKind::Integer:
            return ReadNumber<std::int64_t>(text);
        case PropertyKind::Number:
            return ReadNumber<double>(text);
        case PropertyKind::ObjectReference:
            return SceneObjectReference{ReadNumber<SceneObjectId>(text)};
        case PropertyKind::AssetReference:
            return AssetReference{text};
        ...
        case PropertyKind::Color: {
            ...
            if (kind == PropertyKind::Vector2) {
                ...
                if (!(stream >> result.x >> comma) || comma != ',' || !(stream >> result.y) ||
                    ...
                    throw std::invalid_argument("Enter x, y");
                stream >> std::ws;
                if (!stream.eof()) throw std::invalid_argument("Enter x, y");
                return result;
            }
            int channels[4];
            for (int i = 0; i < 4; ++i) {
                if (i && (!(stream >> comma) || comma != ',')) throw std::invalid_argument("Enter r, g, b, a (0–255)");
                if (!(stream >> channels[i]) || channels[i] < 0 || channels[i] > 255)
                    throw std::invalid_argument("Enter r, g, b, a (0–255)");
            }
            stream >> std::ws;
            if (!stream.eof()) throw std::invalid_argument("Enter r, g, b, a (0–255)");
...
inline View SchemaInspector(std::string key, const SceneComponentTypeDescriptor& schema, const PropertyMap& values,
                            PropertyCommit commit, PropertyError error = {}) {
    ...
    for (const auto& property : schema.properties) {
        ...
        if (!IsPropertyValueCompatible(property.kind, value))
            throw std::invalid_argument("Inspector value does not match its schema");
        auto assign = [property, commit, error](const PropertyValue& value) {
            if (!property.editable) return;
            std::string message;
            if (!ValidatePropertyValue(property, value, &message)) {
                if (error) error(property.key, message);
                return;
            }
            if (commit) commit(property.key, value);
        };
        View control =
            views::Input("value", detail::PropertyText(value), [property, assign, error](const std::string& text) {
                PropertyValue parsed;
                try {
                    parsed = detail::ParseProperty(property.kind, text);
                } catch (const std::invalid_argument& exception) {
                    if (error) error(property.key, exception.what());
                    return;
                }
                assign(parsed);
            });
        ...
        rows.push_back(
            views::Column(property.key, {views::Text("label", label).FitHeight(), std::move(control)}).Spacing(3));
    }
