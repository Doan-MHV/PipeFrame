...
struct View {
    enum class Kind { Row, Column, Text, Button, Input, Slider, Scroll, Stack, Popup, Stateful, Progress, Chart, NumberField, TextField, Mesh, Wrap };
    Kind kind;
    std::string key, text;
    ...
    View &Leading(bool value=true) { leading=value; return *this; }
    struct Series { std::vector<Vector2f> samples; Color color{100,180,255}; };
    std::vector<Series> series;
    struct MeshLayer { std::vector<Vertex2D> vertices; TextureHandle texture; };
    std::vector<MeshLayer> mesh;
    std::shared_ptr<GraphicsResourceService> resources;
    View &Selected(bool value=true) { selected=value; return *this; }
    View &Surface(bool value=true) { surface=value; return *this; }
    float spacing{6};
    float height{34};
    ...
    View &Height(float value) { height=value; fitHeight=false; stretchHeight=false; return *this; }
    View &Width(float value) { width=value; return *this; }
    View &Padding(float value) { padding=value; return *this; }
    View &Spacing(float value) { spacing=value; return *this; }
    View &Expanded(float weight=1) { flex=weight; return *this; }
    View &FitHeight() { fitHeight=true; return *this; }
    View &FillHeight() { stretchHeight=true; return *this; }
    View &Enabled(bool value) { enabled = value; return *this; }
};
...
inline void ValidateView(const View &view) {
    ...
    if (!std::isfinite(view.height) || view.height < 0 ||
        !std::isfinite(view.spacing) || view.spacing < 0 ||
        !std::isfinite(view.width) || view.width < 0 ||
        !std::isfinite(view.padding) || view.padding < 0 ||
    ...
    const bool container = view.kind == View::Kind::Row || view.kind == View::Kind::Column ||
        view.kind == View::Kind::Wrap || view.kind == View::Kind::Scroll || view.kind == View::Kind::Stack || view.kind == View::Kind::Popup;
    ...
    if (view.kind == View::Kind::Stateful && !view.source)
        throw std::invalid_argument("Stateful view needs a source");
    ...
    if (view.kind == View::Kind::Slider && (!std::isfinite(view.minimum) || !std::isfinite(view.maximum) ||
        !std::isfinite(view.value) || !std::isfinite(view.step) || view.step < 0 || view.maximum < view.minimum))
...
inline View Button(std::string key, std::string text, std::function<void()> onPressed) {
    ...
    result.fitHeight=true; return result;
}
inline View Toggle(std::string key, std::string label, bool value, std::function<void(bool)> onChanged) {
    return Button(std::move(key), std::move(label) + (value ? " ON" : " OFF"),
        [value, onChanged = std::move(onChanged)] { if (onChanged) onChanged(!value); });
}
inline View Text(std::string key, std::string text) {
    return {View::Kind::Text, std::move(key), std::move(text)};
}
inline View Mesh(std::string key, std::vector<View::MeshLayer> layers,std::shared_ptr<GraphicsResourceService> resources) {
    View result{View::Kind::Mesh,std::move(key)};
    result.mesh=std::move(layers); result.resources=std::move(resources); result.height=100; return result;
}
inline View Progress(std::string key, float value) {
    View result{View::Kind::Progress,std::move(key)}; result.value=value; result.height=10; return result;
}
inline View Chart(std::string key, std::vector<View::Series> series) {
    View result{View::Kind::Chart,std::move(key)}; result.series=std::move(series); result.height=90; return result;
}
...
inline View Row(std::string key, std::vector<View> children) {
    View result{View::Kind::Row, std::move(key), {}, {}, std::move(children)}; result.fitHeight=true; return result;
}
inline View Wrap(std::string key,std::vector<View> children,float minimumWidth=100) {
    ...
    result.minimum=minimumWidth; result.fitHeight=true; return result;
}
inline View Column(std::string key, std::vector<View> children) {
    View result{View::Kind::Column, std::move(key), {}, {}, std::move(children)}; result.fitHeight=true; return result;
}
inline View Card(std::string key, std::string title, std::string value, std::string detail) {
    return Column(std::move(key),{Text("title",std::move(title)).FitHeight(),
        Text("value",std::move(value)).FitHeight(),Text("detail",std::move(detail)).FitHeight()}).Padding(10).Surface();
}
inline View Input(std::string key, std::string value, std::function<void(const std::string &)> commit) {
    ...
    result.onCommitted=std::move(commit); return result;
}
inline View NumberField(std::string key, std::string label, float value, std::function<void(float)> commit) {
    ...
    result.value=value; result.onChanged=std::move(commit); result.fitHeight=true; return result;
}
inline View TextField(std::string key, std::string label, std::string value, std::function<void(const std::string &)> commit) {
    View result{View::Kind::TextField,std::move(key),std::move(label)};
    result.inputValue=std::move(value); result.onCommitted=std::move(commit); result.fitHeight=true; return result;
}
inline View Slider(std::string key, float value, float minimum, float maximum, std::function<void(float)> change) {
    ...
    result.value=value; result.minimum=minimum; result.maximum=maximum; result.onChanged=std::move(change); return result;
}
...
inline View Stateful(std::string key, std::shared_ptr<ViewSource> source) {
    View result{View::Kind::Stateful, std::move(key)}; result.source=std::move(source); return result;
}
