...
public:
    ...
    Subscription Observe(Observer&& observer) {
...
void BindProperty(WidgetType& widget, State<Value>& state, Setter&& setter) {
    ...
    auto subscription = state.Observe([&widget, lifetime = widget.Lifetime(),
                                       retainedSetter = std::move(retainedSetter)](const Value& value) mutable {
...
void BindVisible(WidgetType& widget, State<bool>& state) {
    BindProperty(widget, state, [](WidgetType& target, const bool visible) { target.SetVisible(visible); });
}

template <typename WidgetType>
void BindEnabled(WidgetType& widget, State<bool>& state) {
    BindProperty(widget, state, [](WidgetType& target, const bool enabled) { target.SetEnabled(enabled); });
}
