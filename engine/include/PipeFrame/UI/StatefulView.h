...
template<class State> class StatefulView {
public:
...
private:
    struct Source final : ViewSource, std::enable_shared_from_this<Source> {
            ...
            if (builtRevision != revision) {
                ...
                Setter set=[weak=this->weak_from_this(),lease](std::function<void(State &)> mutation) {
                    if (auto source=weak.lock(); source && source->mounted && source->generation==lease) {
                        auto next=source->state; mutation(next); source->state=std::move(next); ++source->revision;
                    }
                };
                auto next = builder(state,set);
                ValidateView(next);
                view = std::move(next); builtRevision = building;
            }
            return view;
        }
        ...
        void Mount() override {
            ...
            mounted=true;
            try { if (onMount) onMount(); } catch (...) { mounted=false; subscriptions.clear(); throw; }
        }
        void Unmount() noexcept override {
            if (!mounted) return;
            mounted=false; ++generation; subscriptions.clear();
            // Release captures from the rendered description on removal.
            view=View{View::Kind::Column}; builtRevision=0;
            if (onUnmount) { try { onUnmount(); } catch (...) { /* destruction must complete */ } }
        }
    };
public:
    using Builder = std::function<View(const State &)>;
    StatefulView(State initial, Builder builder) : StatefulView(std::move(initial),
    ...
    StatefulView(State initial, ReactiveBuilder builder) : source(std::make_shared<Source>(std::move(initial),std::move(builder))) {
    ...
    template<class Mutation> void SetState(Mutation mutation) {
        auto next=source->state; mutation(next); source->state=std::move(next); ++source->revision;
    }
    const View &Build() { return source->Build(); }
    template<class Handle> void RetainForMount(Handle &&handle) {
    ...
    void SetLifecycle(std::function<void()> mount, std::function<void()> unmount) {
        ...
        source->onMount=std::move(mount); source->onUnmount=std::move(unmount);
    }
private:
