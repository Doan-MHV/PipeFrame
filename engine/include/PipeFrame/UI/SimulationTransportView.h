...
inline View SimulationTransportView(std::string key,const SimulationTransportModel &model) {
    ...
    for(int i=0;i<4;++i) speeds.push_back(views::Button("speed:"+std::to_string(i),names[i],
        [callback=model.selectSpeed,i]{if(callback)callback(static_cast<SimulationSpeed>(i));}).Selected(static_cast<int>(model.speed)==i));
    return views::Column(std::move(key),{views::Text("label","SIMULATION").FitHeight(),
        views::Row("actions",{views::Button("play",model.playing?"PAUSE":"PLAY",model.playPause),
        ...
        views::Row("speeds",std::move(speeds))}).Padding(8).Surface();
}
}
