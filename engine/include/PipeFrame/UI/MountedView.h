...
public:
    ...
    bool IsMounted() const { const auto value=state.lock(); return value && value->active; }
    void SetView(View view) const {
        ValidateView(view);
        auto value=Require(); value->view=std::move(view); ++value->revision;
    }
    void SetBounds(float x, float y, float width, float height) const {
        ...
        auto value=Require(); value->x=x; value->y=y; value->width=width; value->height=height;
    }
    void Unmount() const { if (auto value=state.lock()) value->active=false; }
private:
