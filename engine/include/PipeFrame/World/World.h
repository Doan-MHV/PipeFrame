...
class World {
  ...
  protected:
    ...
    template <class F> void RunPhase(std::size_t phase, float delta, F run) { sequence.Execute(phase, delta, run); }
...
class RuntimeWorld {
  public:
    ...
    BehaviourScene &GetScene() { return scene; }
