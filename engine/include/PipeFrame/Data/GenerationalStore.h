...
public:
    ...
    T* Get(HandleType handle) {
        ...
        return slot ? &values[slot->denseIndex] : nullptr;
    }
...
private:
    ...
    Slot* FindSlot(HandleType h) {
        return h.index < slots.size() && slots[h.index].occupied && slots[h.index].generation == h.generation
                   ? &slots[h.index]
                   : nullptr;
    }
    const Slot* FindSlot(HandleType h) const {
        return h.index < slots.size() && slots[h.index].occupied && slots[h.index].generation == h.generation
                   ? &slots[h.index]
                   : nullptr;
    }
