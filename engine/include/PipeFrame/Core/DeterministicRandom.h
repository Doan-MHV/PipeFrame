...
public:
    ...
    [[nodiscard]] std::uint64_t NextU64() { state+=0x9e3779b97f4a7c15ULL; return Mix(state); }
    ...
    [[nodiscard]] DeterministicRandom Stream(std::uint64_t streamId) const { return DeterministicRandom(Mix(rootSeed^streamId)); }
private:
    static std::uint64_t Mix(std::uint64_t value) { value=(value^(value>>30))*0xbf58476d1ce4e5b9ULL; value=(value^(value>>27))*0x94d049bb133111ebULL; return value^(value>>31); }
