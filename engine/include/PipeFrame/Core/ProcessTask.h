...
#include <vector>
namespace pipeframe {
struct ProcessCommand { std::string label; std::vector<std::string> arguments; };
struct ProcessProgress { bool running{},cancelled{}; int exitCode{}; std::string stage,output; };
...
private:struct State;std::unique_ptr<State> state;
};
}
