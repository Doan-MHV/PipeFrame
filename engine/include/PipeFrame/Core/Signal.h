...
public:
    ...
    Token Connect(std::function<void(Args...)> callback) {
    ...
    void Emit(Args... args) const {
        const auto copy = callbacks;
        for (const auto& [token, callback] : copy)
            if (callbacks.contains(token)) callback(args...);
    }
    [[nodiscard]] std::size_t ListenerCount() const { return callbacks.size(); }

private:
    Token nextToken{1};
    std::unordered_map<Token, std::function<void(Args...)>> callbacks;
};
}  // namespace pipeframe
#endif
