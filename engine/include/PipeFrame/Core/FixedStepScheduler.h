...
public:
    explicit FixedStepScheduler(double stepSeconds, std::size_t maximumSteps = 8)
        : step(stepSeconds), maximum(maximumSteps) {}
    template <typename Function>
    std::size_t Advance(double elapsedSeconds, Function function) {
        accumulator += std::max(0.0, elapsedSeconds);
        std::size_t count = 0;
        while (accumulator + 1e-12 >= step && count < maximum) {
            function(step);
            accumulator -= step;
            ++count;
        }
