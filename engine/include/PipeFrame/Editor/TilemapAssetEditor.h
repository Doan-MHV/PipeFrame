...
public:
    ...
    void SetEnabled(bool enabled) override { if (tool) tool->SetEnabled(enabled); }
    void SetBounds(std::optional<Rectanglef> bounds) { paintBounds=bounds;if(tool)tool->SetBounds(bounds); }
    ...
    std::optional<std::pair<GridCoordinate,GridCoordinate>> Measurement() const { return tool?tool->Measurement():std::nullopt; }
    ...
    void ClearBrushes(){
        ...
        if(tool){tool->SetEnabled(false);tool->Configure(tool->Layer(),std::make_shared<TileBrush>(0),tool->Shape());}
        activeBrush.reset();brushes.clear();
    }
    void SetBrushes(std::vector<BrushToolDescriptor> value){ClearBrushes();brushes=std::move(value);}
    bool SelectBrush(const std::string &id){
        if(!map||!tool)return Fail("Open a map first");
        tool->SetEnabled(false);
        if(id.empty()){activeBrush.reset();tool->Configure(tool->Layer(),std::make_shared<TileBrush>(0),tool->Shape());return true;}
        for(const auto &descriptor:brushes)if(descriptor.id==id){
            std::shared_ptr<BrushTool> next;
            try{next=descriptor.create();}catch(const std::exception &e){return Fail(e.what());}catch(...){return Fail("Brush factory failed");}
            if(!next)return Fail("Brush factory returned no brush");
            if(!next->DataTarget().empty()&&!map->DataLayer(next->DataTarget())){
                try{
                    auto initialized=*map;initialized.AddDataLayer(std::string(next->DataTarget()),next->DataMinimum(),next->DataMaximum());
                    for(int y=0;y<map->Rows();++y)for(int x=0;x<map->Columns();++x)
                        if(!initialized.SetData(next->DataTarget(),{x,y},next->InitialData(*map,{x,y})))return Fail("Invalid brush layer initialization");
                    if(!Replace(std::move(initialized)))return false;
                }catch(const std::exception &e){return Fail(e.what());}
            }
            activeBrush=std::move(next);tool->Configure(tool->Layer(),activeBrush,tool->Shape());tool->SetEnabled(true);error.clear();return true;
        }
    ...
    bool SetBrushSetting(const std::string &key,const PropertyValue &value){
        ...
        tool->Cancel();return activeBrush->SetSetting(key,value,error);
    }
    void ConfigureGesture(std::size_t layer,TileId tile,TilemapPaintShape shape){
        ...
        if(!effect)effect=std::make_shared<TileBrush>(tile);
        else if(tile==0&&!activeBrush->DataTarget().empty())effect=std::make_shared<EraseDataBrush>(std::string(activeBrush->DataTarget()));
        Configure(layer,std::move(effect),shape);
    }
    bool Open(assets::AssetDatabase &assets, const assets::AssetId &id) {
        error.clear();
        if (IsDirty() || HasPointerCapture()) return Fail("Save or discard the active tilemap before opening another asset");
        const auto *record = assets.Find(id);
        if (!record || record->type != assets::AssetType::Tilemap)
            return Fail("Select a tilemap asset");
        ...
        Close(true);
        database = &assets; projectRoot = assets.GetProjectDirectory(); assetId = id;
        sourcePath = record->sourcePath; baseline = std::move(contents);
    ...
    bool Close(bool discard = false) {
        ...
        ClearBrushes();tool.reset(); map.reset();paintBounds.reset(); resizePlan.reset();resizeSummary.clear();history.clear(); cursor = 0; savedCursor = 0;
        database = nullptr; projectRoot.clear(); sourcePath.clear(); assetId.clear(); baseline.clear();
        return true;
    }
    ...
    bool AddDataLayer(std::string id,double minimum,double maximum){
        ...
        try{auto next=*map;next.AddDataLayer(std::move(id),minimum,maximum);return Replace(std::move(next));}
        catch(const std::exception &e){return Fail(e.what());}
    }
    bool HandleEvent(const InputEvent &event, std::optional<GridCoordinate> cell = {}, bool mayBegin = false) {
        ...
        if (result.committed) {
            ...
            history.push_back(std::move(*result.committed)); ++cursor;
        }
        error = tool->LastError();
        return result.consumed;
    }
    void SetBrushOptions(int radius,bool axis){if(tool)try{tool->SetBrushOptions(radius,axis);error.clear();}catch(const std::exception &e){Fail(e.what());}}
    ...
    void ClearHover(){if(tool)tool->ClearHover();}
    ...
    bool EditLayer(std::string action,std::size_t index,std::string name={}) {
        ...
        try {
            auto next=*map;
            if(action=="add")next.AddLayer(name.empty()?"Layer "+std::to_string(next.Layers().size()+1):name);
            else if(action=="rename")next.RenameLayer(index,std::move(name));
            else if(action=="remove")next.RemoveLayer(index);
            else if(action=="up")next.MoveLayer(index,index+1);
            else if(action=="down") {if(index==0)return Fail("Layer is already at the bottom");next.MoveLayer(index,index-1);}
            else {
                const auto layer=next.Layers().at(index);
                if(action!="visible"&&action!="collision"&&action!="lock")return Fail("Unknown layer operation");
                next.SetLayerFlags(index,action=="visible"?!layer.visible:layer.visible,
                    action=="collision"?!layer.collision:layer.collision,action=="lock"?!layer.locked:layer.locked);
            }
            auto active=tool->Layer();
            if(action=="add")active=next.Layers().size()-1;
            else if(action=="remove"&&index<active)--active;
            else if(action=="up"){if(active==index)++active;else if(active==index+1)--active;}
            else if(action=="down"){if(active==index)--active;else if(active+1==index)++active;}
            tool->SetLayer(std::min(active,next.Layers().size()-1));return Replace(std::move(next));
        }catch(const std::exception &e){return Fail(e.what());}
    }
    bool DefineTile(TileId id,Color tint,bool solid){
        if(!map||id==0)return Fail("Tile IDs start at 1");
        if(!map->Tileset().empty()&&!map->Definition(id))return Fail("Atlas tile definitions come from its tileset");
        auto next=*map;next.DefineTile({id,tint,solid});return Replace(std::move(next));
    }
    bool PaintSelection(TileId tile,bool boundary=false){
        ...
        try {
            ...
            TilemapEdit edit(*map,tool->Layer(),paintBounds,bool(activeBrush));const TileBrush tileBrush(tile);
            class ClearData final:public EnvironmentBrush{public:explicit ClearData(std::string_view id):id(id){} std::string_view DataTarget()const override{return id;} double PaintData(GridCoordinate,double)const override{return 0;}private:std::string id;};
            const ClearData clear(activeBrush?activeBrush->DataTarget():std::string_view{});
            const EnvironmentBrush &brush=activeBrush?(tile==0?static_cast<const EnvironmentBrush &>(clear):*activeBrush):static_cast<const EnvironmentBrush &>(tileBrush);
            auto range=tool->Selection();
            if(boundary){
                auto bounds=paintBounds.value_or(map->Bounds());
                const auto origin=map->Origin();const auto size=map->CellSize();
                range=GridCellRange{{std::max(0,int(std::floor((bounds.position.x-origin.x)/size))),std::max(0,int(std::floor((bounds.position.y-origin.y)/size)))},
                    {std::min(map->Columns()-1,int(std::ceil((bounds.position.x+bounds.size.x-origin.x)/size))-1),std::min(map->Rows()-1,int(std::ceil((bounds.position.y+bounds.size.y-origin.y)/size))-1)}};
            }
            if(!range)return Fail("Drag a selection first");
            edit.Rectangle(range->minimum,range->maximum,brush,!boundary);auto patch=edit.Preview();
            if(patch.Empty())return true;
            if(!edit.Commit(*map))return Fail("Map changed during selection edit");
            if(savedCursor&&*savedCursor>cursor)savedCursor.reset();history.resize(cursor);history.emplace_back(std::move(patch));++cursor;error.clear();return true;
        }catch(const std::exception &e){return Fail(e.what());}
    }
    ...
    bool AssignTileset(const std::string &assetId) {
        ...
        if(!record||record->type!=assets::AssetType::Tileset||record->state!=assets::AssetState::Ready)return Fail("Choose a ready tileset");
        VisualAssetEditor validation;
        if(!validation.Open(*database,assetId)||!validation.ValidateReferences())return Fail(validation.LastError());
        std::ifstream input(projectRoot/record->sourcePath);auto atlas=LoadVisualAsset<Tileset2D>(input,error);if(!atlas)return false;
        auto next=*map;const auto count=atlas->columns*atlas->rows;
        for(const auto &[id,tile]:next.Definitions())if(id>count)return Fail("Tileset has fewer tiles than the map definitions");
        next.SetTileset(assetId);
        for(std::uint32_t id=1;id<=count;++id)if(!next.Definition(id))next.DefineTile({id,{255,255,255,255},false});
        return Replace(std::move(next));
    }
    bool PrepareResize(int columns,int rows,float cellSize,bool resample=false) {
        resizePlan.reset();resizeSummary.clear();
        ...
        try {
            if(resample){
                ...
                if(std::abs(cellSize*rows-map->Bounds().size.y)>0.001f)return Fail("Keep the map aspect ratio for square-cell resampling");
            }
            Tilemap2D next(columns,rows,cellSize,map->Origin());next.SetTileset(map->Tileset());
            for(const auto &[id,tile]:map->Definitions())next.DefineTile(tile);
            std::size_t dropped=0;
            for(std::size_t i=0;i<map->Layers().size();++i){
                const auto &layer=map->Layers()[i];next.AddLayer(layer.name);
                for(int y=0;y<rows;++y)for(int x=0;x<columns;++x){
                    const GridCoordinate old=resample?GridCoordinate{std::min(map->Columns()-1,int((x+.5)*map->Columns()/columns)),std::min(map->Rows()-1,int((y+.5)*map->Rows()/rows))}:GridCoordinate{x,y};
                    if(map->Contains(old))next.SetTile(i,{x,y},map->Tile(i,old));
                }
                if(!resample)for(int y=0;y<map->Rows();++y)for(int x=0;x<map->Columns();++x)
            ...
            resizeRevision=map->Revision();resizePlan=std::move(next);
            resizeSummary=std::to_string(columns)+" x "+std::to_string(rows)+"; "+(resample?"resample same area":std::to_string(dropped)+" painted cells cropped");
            error.clear();return true;
        }catch(const std::exception &failure){resizePlan.reset();return Fail(failure.what());}
    }
    std::optional<Rectanglef> ResizeBounds()const{return resizePlan?std::optional{resizePlan->Bounds()}:std::nullopt;}
    const std::string &ResizeSummary()const{return resizeSummary;}
    bool ApplyResize(){
        if(!resizePlan||!map||map->Revision()!=resizeRevision||HasPointerCapture())return Fail("Preview the current map before applying resize");
        auto next=std::move(*resizePlan);resizePlan.reset();resizeSummary.clear();return Replace(std::move(next));
    }
    ...
    std::optional<assets::AssetId> CopyAsset() {
        ...
        if (!map || !database || database->GetProjectDirectory()!=projectRoot) {
            Fail("Open a map in the active project first"); return {};
        }
        if(database->HasActiveImport()){Fail("Wait for the active import before copying");return {};}
        if (IsDirty() || HasPointerCapture()) {
            Fail("Save the map and finish the stroke before making it unique"); return {};
        }
        std::string contents;
        const auto *record=database->Find(assetId);
        if(!record || record->sourcePath!=sourcePath || !Read(projectRoot/sourcePath,contents) || contents!=baseline) {
            Fail("Map source changed; reopen it before making a copy"); return {};
        }
        const auto folder=projectRoot/"Assets/Tilemaps";
        std::error_code ec;std::filesystem::create_directories(folder,ec);
        if(ec){Fail("Cannot create map asset folder");return {};}
        ...
        {std::ofstream output(destination,std::ios::binary);output<<contents;output.close();
            if(!output){std::filesystem::remove(destination,ec);Fail("Cannot write map copy");return {};}}
    ...
    bool Save() {
        ...
        if (std::filesystem::exists(temporary, ec) || ec) return Fail("Tilemap save temporary file is already present");
        {
            ...
            file << output.str(); file.close();
            if (!file) { std::filesystem::remove(temporary, ec); return Fail("Cannot write tilemap source"); }
        }
        std::filesystem::rename(temporary, path, ec);
        if (ec) { std::filesystem::remove(temporary, ec); return Fail("Cannot replace tilemap source"); }
        baseline = output.str();
        savedCursor.reset();
...
private:
    struct Snapshot {Tilemap2D before,after;};
    bool Replace(Tilemap2D next){
        tool->ClearSelection();if(savedCursor&&*savedCursor>cursor)savedCursor.reset();
        history.erase(history.begin()+cursor,history.end());history.emplace_back(Snapshot{*map,std::move(next)});
        map->ReplaceWith(std::get<Snapshot>(history.back()).after);++cursor;error.clear();return true;
    }
    bool ApplyHistory(std::size_t index,bool forward){
        return std::visit([&](const auto &entry){using T=std::decay_t<decltype(entry)>;
            if constexpr(std::is_same_v<T,TilemapPatch>)return entry.Apply(*map,forward);
            else {map->ReplaceWith(forward?entry.after:entry.before);tool->SetLayer(std::min(tool->Layer(),map->Layers().size()-1));return true;}
        },history[index]);
    }
    static bool Read(const std::filesystem::path &path, std::string &contents) {
        ...
        contents = buffer.str(); return true;
    }
    ...
    std::optional<Tilemap2D> resizePlan;std::uint64_t resizeRevision{};std::string resizeSummary;
    std::size_t cursor{};
