...
public:
    ...
    ~TilemapPaintTool() override {
        SetEnabled(false);
        if (brush) brush->OnTeardown();
    }
    ...
    void Configure(std::size_t targetLayer, std::shared_ptr<const EnvironmentBrush> effect,
        ...
        Cancel();
        if (brush) {
            if (enabled) brush->OnDeactivate();
            if (brush.get() != effect.get()) brush->OnTeardown();
        }
        if (layer != targetLayer) selection.reset();
        sampledTile.reset();
        layer = targetLayer;
        brush = std::move(effect);
        shape = mode;
        if (enabled && brush) brush->OnActivate();
    }
    ...
    std::optional<GridCoordinate> HoverCell() const {
        return HasPointerCapture() ? std::optional<GridCoordinate>{endpoint} : hover;
    }
    std::optional<std::pair<GridCoordinate, GridCoordinate>> Gesture() const {
        return edit ? std::optional{std::pair{anchor, endpoint}} : std::nullopt;
    }
    void SetBounds(std::optional<Rectanglef> value) {
        if (bounds != value) {
            Cancel();
            selection.reset();
            bounds = value;
    ...
    TilemapPatch Preview() const { return edit && document.Revision() == revision ? edit->Preview() : TilemapPatch{}; }
    const std::string& LastError() const { return error; }
    void Cancel() {
        if (edit && brush) brush->OnCancel();
        edit.reset();
        strokeHorizontal.reset();
        visited.clear();
        fillPrepared = false;
        measurement.reset();
    }
    ...
    Result HandleEvent(const InputEvent& event, std::optional<GridCoordinate> cell = {}, bool mayBegin = false) {
        if (!enabled) return {};
        if (event.type == InputEventType::KeyPressed && !HasPointerCapture()) {
            const auto* key = event.GetIf<KeyInput>();
            if (key && key->key == InputKey::L && !key->control && !key->system && !key->alt) {
                axisConstraint = !axisConstraint;
                return {true, {}};
            }
        }
        if (event.type == InputEventType::PointerMoved) hover = mayBegin ? cell : std::nullopt;
        if (event.type == InputEventType::KeyPressed && event.GetIf<KeyInput>() &&
            event.GetIf<KeyInput>()->key == InputKey::Escape && !HasPointerCapture()) {
            const bool had = bool(selection) || bool(measurement);
            ClearSelection();
            return {had, {}};
        }
        const bool captured = HasPointerCapture();
        if (captured && document.Revision() != revision) {
            error = "Tilemap changed during the stroke";
            Cancel();
            return {true, {}};
        }
        if (captured && ((event.type == InputEventType::KeyPressed && event.GetIf<KeyInput>() &&
                          event.GetIf<KeyInput>()->key == InputKey::Escape) ||
                         (event.type == InputEventType::FocusChanged && event.GetIf<FocusInput>() &&
                          !event.GetIf<FocusInput>()->focused) ||
                         (event.type == InputEventType::PointerPresenceChanged && event.GetIf<PointerPresenceInput>() &&
                          !event.GetIf<PointerPresenceInput>()->inside))) {
            Cancel();
            return {true, {}};
        }
        try {
            if (event.type == InputEventType::PointerPressed) {
                ...
                error.clear();
                if (brush->DataTarget().empty() && layer < document.Layers().size() &&
                    !document.Layers()[layer].visible && shape != TilemapPaintShape::Ruler &&
                    shape != TilemapPaintShape::Selection && shape != TilemapPaintShape::Eyedropper) {
                    error = "Show the layer before painting";
                    return {true, {}};
                }
                edit = std::make_unique<TilemapEdit>(
                    document, layer, bounds,
                    !brush->DataTarget().empty() || shape == TilemapPaintShape::Ruler ||
                        shape == TilemapPaintShape::Selection || shape == TilemapPaintShape::Eyedropper);
                if (!edit->Contains(*cell)) {
                    Cancel();
                    return {};
                }
                hover = *cell;
            ...
            if (event.type == InputEventType::PointerReleased) {
                ...
                Update(*cell);
                if (shape == TilemapPaintShape::Selection) {
                    selection = SelectionRange(anchor, endpoint);
                    brush->OnEnd();
                    edit.reset();
                    visited.clear();
                    return {true, {}};
                }
                if (shape == TilemapPaintShape::Ruler) {
                    brush->OnEnd();
                    edit.reset();
                    visited.clear();
                    return {true, {}};
...
private:
    std::optional<GridCellRange> SelectionRange(GridCoordinate a, GridCoordinate b) const {
        int left = std::max(0, std::min(a.column, b.column)),
            right = std::min(document.Columns() - 1, std::max(a.column, b.column));
        ...
        if (bounds) {
            ...
            right = std::min(
                right,
                int(std::ceil((bounds->position.x + bounds->size.x - document.Origin().x) / document.CellSize())) - 1);
            bottom = std::min(
                bottom,
                int(std::ceil((bounds->position.y + bounds->size.y - document.Origin().y) / document.CellSize())) - 1);
        }
    ...
    void NotifyPreview() {
        if (edit && brush) {
            const auto patch = edit->Preview();
            brush->OnPreview(patch.changes.size() + patch.dataChanges.size());
        }
    }
    void Update(GridCoordinate cell) {
        brush->OnUpdate(cell);
        if (axisConstraint) {
            if (!strokeHorizontal && cell != anchor)
                strokeHorizontal = std::abs(cell.column - anchor.column) >= std::abs(cell.row - anchor.row);
            if (strokeHorizontal) {
                if (*strokeHorizontal)
                    cell.row = anchor.row;
                else
        ...
        if (shape == TilemapPaintShape::Pencil) {
            ...
            edit->Line(previous, cell, once, radius);
            previous = cell;
            return;
        }
        ...
        edit->Cancel();
        switch (shape) {
            case TilemapPaintShape::Line:
                edit->Line(anchor, cell, *brush);
                break;
            case TilemapPaintShape::Rectangle:
                edit->Rectangle(anchor, cell, *brush);
                break;
            case TilemapPaintShape::Outline:
                edit->Rectangle(anchor, cell, *brush, false);
                break;
            case TilemapPaintShape::Fill:
                edit->Fill(anchor, *brush);
                fillPrepared = true;
                break;
            case TilemapPaintShape::Circle: {
                const auto dx = double(cell.column) - anchor.column, dy = double(cell.row) - anchor.row;
                edit->Circle(anchor, int(std::min(1000000.0, std::hypot(dx, dy))), *brush);
                break;
            }
            case TilemapPaintShape::Eyedropper:
                sampledTile = document.Tile(layer, anchor);
                break;
            case TilemapPaintShape::Ruler:
                measurement = std::pair{anchor, cell};
                break;
            case TilemapPaintShape::Selection:
                break;
            case TilemapPaintShape::Pencil:
                break;
        }
    }
