...
template<class Derived> class SchemaBrush : public BrushTool {
public:
    SceneComponentTypeDescriptor SettingsSchema()const override{return Derived::Schema().Describe();}
    PropertyMap Settings()const override{return Derived::Schema().Serialize(static_cast<const Derived &>(*this)).properties;}
