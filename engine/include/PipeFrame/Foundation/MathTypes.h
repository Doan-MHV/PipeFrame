...
struct Vector2 {
    ...
    constexpr Vector2 &operator-=(const Vector2 other) { x -= other.x; y -= other.y; return *this; }
    constexpr Vector2 &operator*=(const T scalar) { x *= scalar; y *= scalar; return *this; }
    constexpr Vector2 &operator/=(const T scalar) { x /= scalar; y /= scalar; return *this; }
...
template <typename T> constexpr Vector2<T> operator-(Vector2<T> a, const Vector2<T> b) { return a -= b; }
template <typename T> constexpr Vector2<T> operator-(const Vector2<T> value) { return {-value.x, -value.y}; }
template <typename T> constexpr Vector2<T> operator*(Vector2<T> value, const T scalar) { return value *= scalar; }
template <typename T> constexpr Vector2<T> operator*(const T scalar, Vector2<T> value) { return value *= scalar; }
template <typename T> constexpr Vector2<T> operator/(Vector2<T> value, const T scalar) { return value /= scalar; }
...
struct Color {
    union { std::uint8_t red; std::uint8_t r; };
    union { std::uint8_t green; std::uint8_t g; };
    union { std::uint8_t blue; std::uint8_t b; };
    union { std::uint8_t alpha; std::uint8_t a; };
    constexpr Color(std::uint8_t red=0,std::uint8_t green=0,std::uint8_t blue=0,
                    std::uint8_t alpha=255):red(red),green(green),blue(blue),alpha(alpha){}
...
template <typename T>
[[nodiscard]] inline T LengthSquared(const Vector2<T> value) { return value.x * value.x + value.y * value.y; }

[[nodiscard]] inline float Length(const Vector2f value) { return std::sqrt(LengthSquared(value)); }
