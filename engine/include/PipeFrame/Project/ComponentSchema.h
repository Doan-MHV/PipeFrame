...
public:
    ComponentSchema(std::string id, std::string name)
        : descriptor{std::move(id), std::move(name)} {}
    ...
    ComponentSchema &Accessor(PropertyDescriptor property,
        ...
        descriptor.properties.push_back(std::move(property)); return *this;
    }
    ...
    ComponentSchema &Required() { descriptor.removable = false; return *this; }
    ...
    bool Apply(Component &object, const PropertyMap &values, std::string &error) const {
        for (const auto &[key,value] : values) {
            if (std::ranges::none_of(bindings,[&](const auto &binding) { return binding.property.key==key; })) {
                error="Unknown component field: "+key; return false;
            }
        }
        ...
        for (const auto &binding : bindings) {
            const auto found = values.find(binding.property.key);
            if (found != values.end()) {
                try { binding.write(next, found->second); }
                catch (const std::exception &exception) { error=binding.property.displayName+": "+exception.what(); return false; }
            }
        }
        for (const auto &[message,predicate]:validators) {
            try {
                if (!predicate(next)) { error=message; return false; }
            } catch (const std::exception &) { error=message; return false; }
        }
