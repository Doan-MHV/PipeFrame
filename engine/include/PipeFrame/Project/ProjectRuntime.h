...
public:
    ...
    template <class T>
    void RegisterBrush(std::string category = "Project") {
        const auto schema = T::Schema().Describe();
        for (const auto& brush : environmentBrushes)
            if (brush.id == schema.typeId) throw std::invalid_argument("Duplicate brush ID");
        environmentBrushes.push_back(
            {schema.typeId, schema.displayName, std::move(category), [] { return std::make_shared<T>(); }});
    }
...
public:
    ...
    virtual ProjectPluginDescriptor GetPluginDescriptor() const { return {GetName(), GetName()}; }
    ...
    virtual bool Load(const ProjectRuntimeContext& context, std::string& errorMessage) = 0;

    virtual std::span<const SceneObjectTypeDescriptor> GetSceneObjectTypes() const = 0;

    virtual std::span<const SceneComponentTypeDescriptor> GetSceneComponentTypes() const { return {}; }
    ...
    virtual SceneObjectData CreateDefaultObject(const SceneObjectTypeId& typeId) const {
        SceneObjectData object;

        object.name = typeId;
        object.typeId = typeId;

        for (const SceneObjectTypeDescriptor& type : GetSceneObjectTypes()) {
            ...
            for (const PropertyDescriptor& property : type.properties) {
                object.properties.emplace(property.key, property.defaultValue);
            }
            ...
            for (const std::string& componentTypeId : type.componentTypeIds) {
                for (const SceneComponentTypeDescriptor& componentType : GetAllSceneComponentTypes()) {
                    ...
                    SceneComponentData component{
                        componentType.typeId, componentType.schemaVersion, {}, true, componentType.editorOnly};
    ...
    void NormalizeObjectComponents(SceneObjectData& object) const {
        ...
        for (const auto& objectType : GetSceneObjectTypes())
            if (objectType.typeId == object.typeId) {
                for (const auto& id : objectType.componentTypeIds) {
                    ...
                    if (component == object.components.end()) {
                        object.components.push_back({id, type->schemaVersion});
                        component = std::prev(object.components.end());
                    }
                    for (const auto& field : type->properties) {
                        if (const auto legacy = object.properties.find(field.key); legacy != object.properties.end()) {
                            component->properties.insert_or_assign(field.key, legacy->second);
                            object.properties.erase(legacy);
                        } else
                            component->properties.try_emplace(field.key, field.defaultValue);
                    }
                }
            }
        SynchronizeTransformComponent(object);
    ...
    bool TrySynchronizeComponentValues(std::span<const SceneObjectData> previous, std::span<const SceneObjectData> next,
        ...
        for (const auto& value : next) {
            ...
            comparable.components = old->components;
            comparable.transform = old->transform;
            ...
            for (const auto& component : value.components) {
                ...
                auto same = component;
                same.properties = before->properties;
    ...
    bool ApplyAuthoredComponentEdits(std::vector<SceneObjectData>& authored,
        ...
        for (const auto& edit : edits) {
            ...
            const auto component =
                std::ranges::find(object->components, edit.componentTypeId, &SceneComponentData::typeId);
            if (component == object->components.end()) return false;
            component->properties.insert_or_assign(edit.propertyKey, edit.value);
            auto mutation = std::ranges::find_if(
                mutations, [&](const auto& m) { return m.object == entity && m.typeId == edit.componentTypeId; });
            if (mutation == mutations.end())
                mutations.push_back({entity, edit.componentTypeId, {{edit.propertyKey, edit.value}}});
            else
                mutation->values.insert_or_assign(edit.propertyKey, edit.value);
        }
        std::string error;
        if (!registry->Apply(mutations, error)) return false;
        for (auto& object : next)
            SynchronizeSceneTransform(object);
    ...
    static double ReadNumber(const pipeframe::SceneObjectData& object, const std::string& key,
        ...
        if (!properties->contains(key))
            if (const auto component =
                    std::ranges::find(object.components, object.typeId, &pipeframe::SceneComponentData::typeId);
                component != object.components.end() && component->properties.contains(key))
    ...
    static bool ReadBoolean(const pipeframe::SceneObjectData& object, const std::string& key, const bool defaultValue) {
        const pipeframe::PropertyMap* properties = &object.properties;
        if (!properties->contains(key))
            if (const auto component =
                    std::ranges::find(object.components, object.typeId, &pipeframe::SceneComponentData::typeId);
    ...
    virtual void SynchronizeScene(std::span<const SceneObjectData> objects) = 0;
    ...
    virtual bool ApplyComponentEdits(std::span<const ProjectRuntimeComponentEdit> edits,
                                     ProjectRuntimeAuthoringState state) {
        (void)edits;
        (void)state;
        return false;
    ...
    virtual std::vector<ProjectRuntimeComponentEdit> GetLiveComponentProperties() const { return {}; }

    virtual void SetSelectedObject(std::optional<SceneObjectId> objectId) = 0;

    virtual void SetViewMode(ProjectRuntimeViewMode mode) { (void)mode; }

    virtual std::optional<SceneObjectId> HitTest(Vector2f worldPosition) const = 0;

    virtual void Start() = 0;
    virtual void FixedUpdate(float fixedDeltaTime) = 0;
    virtual void Render(RenderContext& context) = 0;
    virtual void CollectWorldDebug(WorldDebugDraw &, WorldDebugOptions) {}
    virtual void RenderScreen(RenderContext& context) { (void)context; }
    virtual bool ConsumesPointerAt(Vector2i screenPosition, const RenderContext& context) const {
        (void)screenPosition;
        (void)context;
        return false;
    }
    ...
    virtual void HandleEvent(const InputEvent& event, RenderContext& context) {
        (void)event;
        (void)context;
    }

    virtual std::vector<ProjectRuntimeSceneEdit> ConsumeSceneEdits() { return {}; }
    ...
    virtual ProjectRuntimeStatistics GetStatistics() const { return {}; }
};
...
#endif
#else
#define PIPEFRAME_PROJECT_RUNTIME_EXPORT __attribute__((visibility("default")))
#endif

inline constexpr const char* CreateProjectRuntimeSymbol = "PipeFrameCreateProjectRuntime";

inline constexpr const char* DestroyProjectRuntimeSymbol = "PipeFrameDestroyProjectRuntime";

using CreateProjectRuntimeFunction = ProjectRuntime* (*)();

using DestroyProjectRuntimeFunction = void (*)(ProjectRuntime*);
