...
// Version 14 adds the project world-debug collection hook. Rebuild project plugins.
inline constexpr std::uint32_t ProjectPluginAbiVersion = 14;
...
public:
    ...
    bool Invoke(std::string_view id, const ExtensionInvocation& invocation = {}, std::string* error = nullptr) const;
...
public:
    ...
    bool Invoke(std::string_view id, const ActionInvocation& invocation = {}, std::string* error = nullptr) const;
    std::vector<const EditorActionDescriptor*> Search(std::string_view query, std::string_view context = {}) const;
...
public:
    ...
    std::vector<ProjectEvent> Consume() {
...
public:
    ...
    std::vector<GeometryCommand> Consume() {
...
public:
    ...
    std::vector<std::string> Validate() const;
    std::vector<const ProjectSystemDescriptor*> Ordered(SystemPhase phase, std::string* error = nullptr) const;
...
public:
    PluginRegistrar(std::string owner, ExtensionRegistry& extensions, ActionRegistry& actions, SystemRegistry& systems)
