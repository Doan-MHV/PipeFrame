...
public:
    template <class T>
    void Register(ComponentSchema<T> schema) {
        ...
        entries.push_back({typeid(T), descriptor,
                           ...
                           [schema](SceneObject object, const PropertyMap& values, std::function<void()>& commit,
                                    std::string& error) {
                               const auto* current = object.GetComponent<T>();
                               T next = current ? *current : T{};
                               if (!current) {
                                   PropertyMap defaults;
                                   for (const auto& field : schema.Describe().properties)
                                       defaults.emplace(field.key, field.defaultValue);
                               ...
                               commit = [object, next = std::move(next)] {
                                   if (auto* current = object.GetComponent<T>())
                                       *current = next;
                                   else
                                       object.AddComponent<T>(next);
                               };
                               return true;
                           }});
    }
    ...
    std::vector<SceneComponentData> Inspect(SceneObject object) const {
        ...
        for (const auto& entry : entries)
            if (entry.present(object)) result.push_back(entry.serialize(object));
        return result;
    }
    ...
    bool Apply(std::span<const ComponentMutation> mutations, std::string& error, bool editing = true,
               bool validateOnly = false) const {
        ...
        for (std::size_t i = 0; i < mutations.size(); ++i) {
            ...
            if (!mutation.object.IsValid()) {
                error = "Selected entity no longer exists";
                return false;
            }
            for (std::size_t j = 0; j < i; ++j)
                if (mutations[j].object == mutation.object && mutations[j].typeId == mutation.typeId) {
                    error = "Duplicate component mutation";
                    return false;
                }
            const auto entry =
                std::ranges::find(entries, mutation.typeId, [](const Entry& entry) { return entry.descriptor.typeId; });
            if (entry == entries.end() || (editing && !entry->present(mutation.object))) {
                error = "Component is not attached/registered";
                return false;
            }
            for (const auto& [key, value] : mutation.values) {
                ...
                if (field == entry->descriptor.properties.end()) {
                    error = "Unknown component property: " + key;
                    return false;
                }
                if (editing && (!field->editable || field->editorHint == "telemetry")) {
                    error = "Property is read-only: " + key;
                    return false;
                }
