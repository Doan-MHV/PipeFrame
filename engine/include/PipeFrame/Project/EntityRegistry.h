...
public:
    template <class T>
    void RegisterBehaviour(ComponentRegistry& components, std::string id, std::string displayName) {
    ...
    template <class T>
    void Register(SceneObjectTypeDescriptor descriptor, bool editorVisible = true) {
        RegisterFactory<T>(
            std::move(descriptor),
            [](BehaviourScene& scene, const SceneObjectData&) -> SceneObject {
                if constexpr (std::is_default_constructible_v<T>)
                    return scene.Instantiate(T{});
                else
                    throw std::invalid_argument("This entity requires typed spawn arguments");
            },
            editorVisible);
    }
    template <class T, class Factory>
    void RegisterFactory(SceneObjectTypeDescriptor descriptor, Factory factory, bool editorVisible = true) {
    ...
    template <class T>
    SceneObject Spawn(BehaviourScene& scene, const T& recipe) const {
    ...
    void BindFactory(const SceneObjectTypeId& id, Factory factory,
                     std::function<void(SceneObject, const SceneObjectData&)> restored = {}) {
    ...
    std::vector<SceneObjectTypeDescriptor> Describe(bool includeRuntimeOnly = false) const {
        ...
        for (const auto& [id, entry] : entries)
            if (entry.editorVisible || includeRuntimeOnly) result.push_back(entry.descriptor);
        return result;
    }
    SceneObject Instantiate(BehaviourScene& scene, const SceneObjectData& data,
        ...
        try {
            Restore(object, data, components);
            if (found->second.restored) found->second.restored(object, data);
            AttachBehaviours(object, data);
        } catch (...) {
            object.Destroy();
            throw;
        }
    ...
    static void Restore(SceneObject object, const SceneObjectData& data, const ComponentRegistry& components) {
        ...
        for (const auto& component : data.components) {
            if (!components.Contains(component.typeId))
...
public:
    ...
    void Rebuild(BehaviourScene& scene, std::span<const SceneObjectData> source, const EntityRegistry& types,
                 const ComponentRegistry& components) {
        std::map<SceneObjectId, SceneObject> next;
        try {
            for (const auto& data : source)
                if (types.Contains(data.typeId)) {
