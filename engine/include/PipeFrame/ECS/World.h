...
class World {
    ...
    template<class T> struct Pool final : Storage {
        std::vector<T> values;
        ...
        void Remove(Entity entity) override {
            ...
            values.pop_back(); entities.pop_back(); indices.erase(entity);
        }
    };
public:
    ...
    template<class T> ComponentAccess<T> BorrowComponents() {
    ...
    Entity Create() {
        while (alive.contains(next)) ++next;
    ...
    bool Destroy(Entity entity) {
        ...
        for (auto &[type, pool] : pools) pool->Remove(entity);
        return true;
    }
    void Clear() { ++structureRevision; pools.clear(); alive.clear(); revisions.clear(); next = 1; }

    template<class T, class... Args> T &Add(Entity entity, Args&&... args) {
    ...
    template<class T> T *Get(Entity entity) {
    ...
    template<class T> void Remove(Entity entity) { if (auto *pool = FindStorage<T>()) { ++structureRevision; pool->Remove(entity); } }
    template<class T> std::span<T> Components() {
        auto *pool = FindStorage<T>();
        return pool ? std::span<T>(pool->values) : std::span<T>{};
    }
    template<class T> std::span<const T> Components() const {
    ...
    template<class First, class... Rest, class Function> void Each(Function function) {
        ...
        for (Entity entity : EntitiesWith<First>()) {
            ...
            if (first && (... && (Get<Rest>(entity) != nullptr)))
                function(entity, *first, *Get<Rest>(entity)...);
        }
    }
private:
    template<class T> Pool<T> *FindStorage() {
        const auto found = pools.find(typeid(T));
        return found == pools.end() ? nullptr : static_cast<Pool<T> *>(found->second.get());
    }
    template<class T> Pool<T> &StorageFor() {
