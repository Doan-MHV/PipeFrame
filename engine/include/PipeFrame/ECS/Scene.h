...
public:
    ...
    CoroutineId StartCoroutine(Coroutine routine) {
        ...
        tasks.push_back(std::make_unique<Task>(Task{id, std::move(routine)}));
        return id;
    }
    void StopCoroutine(CoroutineId id) {
        for (auto& task : tasks)
            if (task->id == id) task->cancelled = true;
    }
...
private:
    ...
    void AdvanceCoroutines(float delta, Allowed allowed) {
        const auto count = tasks.size();
        for (std::size_t i = 0; i < count && allowed(); ++i) {
            auto& task = *tasks[i];
            if (!task.cancelled && !task.routine.Tick(delta)) task.cancelled = true;
        }
...
public:
    ...
    ~BehaviourScene() {
        try {
            Clear();
        } catch (...) { /* Destructors cannot propagate script exceptions. */
        }
    }
    ...
    ecs::Entity Create() {
        if (clearing) throw std::logic_error("Cannot create during scene clear");
        return world.Create();
    }
    void Create(ecs::Entity entity) {
        if (clearing) throw std::logic_error("Cannot create during scene clear");
        world.Create(entity);
    }
    template <class T, class... Args>
    T& Attach(ecs::Entity entity, Args&&... args) {
        if (!world.IsAlive(entity) || IsPending(entity))
            throw std::invalid_argument("Cannot attach to missing or destroying entity");
    ...
    ecs::Entity GetParent(ecs::Entity entity) const {
        ...
        return world.Revision(entity) == link.childRevision && world.Revision(link.parent) == link.parentRevision
                   ? link.parent
                   : ecs::InvalidEntity;
    }
    ...
    void SetParent(ecs::Entity child, ecs::Entity parent = ecs::InvalidEntity) {
        if (!world.IsAlive(child) || IsPending(child) || (parent && (!world.IsAlive(parent) || IsPending(parent))))
        ...
        if (!parent)
            ...
            parents[child] = {parent, world.Revision(child), world.Revision(parent)};
    }
    ...
    bool HasStartedBehaviour(ecs::Entity entity) const {
        for (const auto& entry : entries)
            if (entry->value->GetEntity() == entity && entry->started) return true;
        return false;
    }
    void SetActive(ecs::Entity entity, bool active) {
        if (!world.IsAlive(entity) || IsPending(entity))
            throw std::invalid_argument("Object does not exist or is being destroyed");
        if (active)
            inactive.erase(entity);
        else
            inactive.insert(entity);
    }
    void SetEnabled(Behaviour& value, bool enabled) {
        for (auto& entry : entries)
            if (entry->value.get() == &value) entry->enabled = enabled;
    }
    ...
    void NotifyContact(ecs::Entity entity, const CollisionEvent2D& event, bool trigger, bool entering) {
        Notify(entity, [&](Behaviour& value) {
            if (trigger) {
                if (entering)
                    value.OnTriggerEnter(event);
                else
                    value.OnTriggerExit(event);
            } else {
                if (entering)
                    value.OnCollisionEnter(event);
                else
                    value.OnCollisionExit(event);
            }
        });
    }
    void NotifyVisibility(ecs::Entity entity, bool visible) {
        Notify(entity, [&](Behaviour& value) {
            if (visible)
                value.OnBecameVisible();
            else
                value.OnBecameInvisible();
        });
    }
...
private:
    ...
    void Notify(ecs::Entity entity, Callback callback) {
        ...
        try {
            ...
            for (std::size_t i = 0; i < count; ++i) {
                ...
                if (entry.value->GetEntity() == entity && entry.started && entry.enabled && IsActive(entity) &&
                    !IsPending(entity))
        ...
        try {
            ...
            for (std::size_t i = 0; i < count; ++i) {
                ...
                if (shouldRun != entry.active) {
                    ...
                    if (entry.active)
                        entry.value->OnEnable();
                    else
                        entry.value->OnDisable();
                }
                ...
                if (fixed) {
                    ...
                    if (entry.enabled && IsActive(entity) && !IsPending(entity))
                        entry.value->AdvanceCoroutines(
                            delta, [&] { return entry.enabled && IsActive(entity) && !IsPending(entity); });
                } else
    ...
    bool IsPending(ecs::Entity entity) const { return pendingIds.contains(entity) || destroying == entity; }
    void QueueSubtree(ecs::Entity root) {
        ...
        while (!stack.empty()) {
            const auto [entity, visited] = stack.back();
            stack.pop_back();
            if (IsPending(entity)) continue;
            if (visited) {
                pending.push_back(entity);
                pendingIds.insert(entity);
                continue;
            }
    ...
    void Flush() {
        ...
        while (!pending.empty()) {
            ...
            for (auto& entry : detached) {
                try {
                    if (entry->active) entry->value->OnDisable();
                } catch (...) {
    ...
    struct ParentLink {
        ecs::Entity parent;
        std::uint64_t childRevision, parentRevision;
    };
    std::unordered_map<ecs::Entity, ParentLink> parents;
};
template <class T>
T* Behaviour::GetService() const {
    return owner ? owner->Services().Find<T>() : nullptr;
}
...
public:
    ...
    std::uint64_t StructureRevision() const {
        RequireValid();
    ...
    T* GetComponent() const {
        RequireValid();
        return scene->Components().Get<T>(entity);
    }
    template <class T, class... Args>
    ...
    template <class T, class... Args>
    T& Attach(Args&&... args) const {
        RequireValid();
        return scene->Attach<T>(entity, std::forward<Args>(args)...);
    }
    ...
    std::vector<SceneObject> GetChildren() const {
        ...
        for (auto child : scene->GetChildren(entity))
            result.push_back(scene->GetObject(child));
        return result;
    }
    ...
    bool IsActiveSelf() const {
        RequireValid();
        return scene->IsActiveSelf(entity);
    }
    bool IsActive() const {
        RequireValid();
        return scene->IsActive(entity);
    }
...
inline SceneObject BehaviourScene::CreateObject() {
