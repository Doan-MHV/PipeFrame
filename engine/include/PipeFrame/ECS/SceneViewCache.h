...
template<class View, class Tag> class SceneViewCache {
public:
    ...
    std::span<View> Items() const { Refresh(); return views; }
    std::size_t Size() const { return GetWorld().template Components<Tag>().size(); }
    template<class Predicate> std::size_t RemoveWhere(Predicate predicate) {
        std::vector<ecs::Entity> removed;
        for (const auto &view : Items()) if (predicate(view)) removed.push_back(view.GetId());
        for (auto id : removed) scene.Destroy(id);
        return removed.size();
    }
    void Clear() {
        for (auto id : GetWorld().template EntitiesWith<Tag>()) scene.Destroy(id);
        Refresh();
    }
    void Refresh() const {
        if (revision == GetWorld().StructureRevision()) return;
        views.clear(); indices.clear();
