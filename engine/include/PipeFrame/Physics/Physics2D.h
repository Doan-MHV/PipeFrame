...
bool ConstrainToBounds(Body& body, Rectanglef bounds) {
    ...
    body.position.x =
        std::clamp(body.position.x, bounds.position.x + body.radius, bounds.position.x + bounds.size.x - body.radius);
    body.position.y =
        std::clamp(body.position.y, bounds.position.y + body.radius, bounds.position.y + bounds.size.y - body.radius);
...
std::optional<Contact2D> SolveCircleContact(First& first, Second& second) {
    ...
    const Vector2f normal =
        distance > 0.0f ? difference / distance : (first.id < second.id ? Vector2f{-1.0f, 0.0f} : Vector2f{1.0f, 0.0f});
    ...
    second.previousPosition = second.position;
...
public:
    void Prepare(Rectanglef bounds, float cellSize) {
        if (!initialized || previousBounds.position != bounds.position || previousBounds.size != bounds.size ||
...
std::size_t SolveCircleContacts(std::span<Body> bodies, Rectanglef bounds, CircleContactWorkspace& workspace,
                                OnContact onContact) {
    if (bodies.empty()) return 0;
    float maximumRadius = 0.001f;
    for (const auto& body : bodies)
        maximumRadius = std::max(maximumRadius, body.radius);
    ...
    for (std::size_t index = 0; index < bodies.size(); ++index)
        entries.push_back({index, bodies[index].position});
        ...
        for (int row = range.minimum.row; row <= range.maximum.row; ++row) {
            for (int column = range.minimum.column; column <= range.maximum.column; ++column) {
                for (const std::size_t second : broadphase.GetIds({column, row})) {
                    ...
                    if (auto contact = SolveCircleContact(bodies[first], bodies[second])) {
                        onContact(*contact);
                        ++count;
                    }
                }
...
[[nodiscard]] inline Vector2f ClosestPoint(Segment2D segment, Vector2f point) {
    ...
    const float projection = std::clamp(
        ((point.x - segment.start.x) * edge.x + (point.y - segment.start.y) * edge.y) / lengthSquared, 0.0f, 1.0f);
...
void VisitBodyDebugGeometry(std::span<const Body> bodies, ColorFor colorFor, Visitor visitor) {
    for (const auto& body : bodies)
        visitor(DebugCircle2D{{body.position, body.radius}, colorFor(body)});
}
}  // namespace pipeframe
#endif
