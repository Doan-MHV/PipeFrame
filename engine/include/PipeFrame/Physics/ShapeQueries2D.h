...
inline float ShapeDot(Vector2f a, Vector2f b) {
    return a.x * b.x + a.y * b.y;
}
inline bool Finite(Vector2f p) {
    return std::isfinite(p.x) && std::isfinite(p.y);
}
inline Vector2f ClosestOnSegment(Vector2f p, Vector2f a, Vector2f b) {
    const auto d = b - a;
    const auto n = LengthSquared(d);
    return a + d * (n > 0 ? std::clamp(ShapeDot(p - a, d) / n, 0.f, 1.f) : 0.f);
}
inline bool InsideShape(Vector2f p, const EnvironmentShape2D& shape) {
    if (shape.segment) return false;
    bool positive = false, negative = false;
    for (int i = 0; i < 4; ++i) {
        const auto a = shape.points[i], d = shape.points[(i + 1) % 4] - a, q = p - a;
        const float cross = d.x * q.y - d.y * q.x;
...
inline std::optional<ShapeHit2D> OverlapShape(Circle2D circle, const EnvironmentShape2D& shape) {
    ...
    for (int i = 0; i < (shape.segment ? 1 : 4); ++i) {
        auto p = ClosestOnSegment(circle.center, shape.points[i], shape.points[(i + 1) % 4]);
        float d = LengthSquared(circle.center - p);
        if (d < best) {
            best = d;
            closest = p;
        }
...
inline std::optional<ShapeHit2D> SweepShape(Circle2D circle, Vector2f delta, const EnvironmentShape2D& shape) {
    ...
    if (InsideShape(circle.center, shape)) return ShapeHit2D{0, circle.center, {}};
    if (auto overlap = OverlapShape(circle, shape);
        overlap && LengthSquared(circle.center - overlap->point) < circle.radius * circle.radius)
    ...
    for (int i = 0; i < (shape.segment ? 1 : 4); ++i) {
        ...
        if (length > 0) {
            ...
            const Vector2f perpendicular{-axis.y, axis.x};
        ...
        for (const auto corner : {a, b}) {
            const auto q = circle.center - corner;
            const double bb = double(q.x) * delta.x + double(q.y) * delta.y;
...
inline std::optional<ShapeHit2D> RaycastShape(Vector2f origin, Vector2f direction, float distance,
                                              const EnvironmentShape2D& shape) {
    for (const auto p : shape.points)
        if (!Finite(p)) return {};
    ...
    if (shape.segment) {
        ...
        if (cross == 0) {
            ...
            if (t > distance || std::max(ShapeDot(a - origin, direction), ShapeDot(b - origin, direction)) < 0)
                return {};
            return ShapeHit2D{distance > 0 ? t / distance : 0, origin + direction * t, {}};
        }
    }
    if (distance == 0) {
        if (auto hit = OverlapShape({origin, 0}, shape)) return hit;
        return {};
    }
