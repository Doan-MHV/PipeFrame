...
namespace pipeframe {
struct GridShapeHit2D {
    GridCoordinate cell{};
    ShapeHit2D contact;
};
// Borrows the existing simulation grid. No mirrored wall array or per-agent allocation.
template <class Blocked>
std::optional<GridShapeHit2D> SweepCircleGrid(Circle2D circle, Vector2f delta, Vector2f origin, float size, int columns,
                                              int rows, Blocked blocked) {
    if (!Finite(circle.center) || !Finite(delta) || !std::isfinite(circle.radius) || circle.radius < 0 ||
        !Finite(origin) || !std::isfinite(size) || size <= 0 || columns <= 0 || rows <= 0)
        return {};
    const auto to = circle.center + delta;
    if (!Finite(to)) return {};
    const auto index = [&](double value, double base, int count) {
        return int(std::clamp(std::floor((value - base) / size), 0.0, double(count - 1)));
    };
...
CircleMotion2D MoveCircle(Circle2D circle, Vector2f displacement, Vector2f velocity, Query query) {
    ...
    for (int i = 0; i < 4 && LengthSquared(displacement) > 1e-12f; ++i) {
        ...
        const float fraction = std::clamp(hit->fraction, 0.f, 1.f);
        result.position += displacement * fraction;
        displacement *= 1 - fraction;
        if (LengthSquared(hit->normal) == 0) {
            result.velocity = {};
            break;
        }
        const float into = ShapeDot(displacement, hit->normal);
        if (into < 0) displacement -= hit->normal * into;
        const float speed = ShapeDot(result.velocity, hit->normal);
        if (speed < 0) result.velocity -= hit->normal * speed;
    }
    return result;
}
}  // namespace pipeframe
