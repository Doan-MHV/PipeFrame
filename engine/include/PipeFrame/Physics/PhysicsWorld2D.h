...
public:
    ...
    std::size_t Step(float deltaTime, float maximumMove = std::numeric_limits<float>::infinity()) {
        for (auto& body : storage.Bodies()) {
            IntegrateBody(body, deltaTime, maximumMove);
            ConstrainToBounds(body, bounds);
        }
        return SolveCircleContacts<PhysicsBody2D>(storage.Bodies(), bounds, contacts, [](const Contact2D&) {});
    }
    std::span<PhysicsBody2D> Bodies() { return storage.Bodies(); }
    std::span<const PhysicsBody2D> Bodies() const { return storage.Bodies(); }
    void Clear() { storage.Clear(); }

private:
    Rectanglef bounds;
    BodyStorage<> storage;
    CircleContactWorkspace contacts;
};
}  // namespace pipeframe
#endif
