...
public:
    struct Resource { std::uint64_t revision{}; std::optional<Tilemap2D> map; GeometryCommand geometry; std::optional<Tileset2D> atlas; std::string error; TilemapChunkCache chunks; };
    ...
    bool Load(const ProjectRuntimeContext &context,std::string &error)override {
        Unload();database=context.services?context.services->Find<assets::AssetDatabase>():nullptr;
        log=context.log;error.clear();return true;
    }
    void Unload()override{cache.clear();database=nullptr;log=nullptr;}
    const Resource *Resolve(const AssetReference &reference) {
        ...
        const auto fail=[&](std::string message)->const Resource * {
            ...
            resource.error=std::move(message);resource.map.reset();resource.geometry.vertices.clear();return &resource;
        };
        ...
        std::string line;
        if(!std::getline(input,line)||line!="PIPEFRAME_COMPILED_ASSET 1")return fail("Invalid tilemap cache header: "+reference.assetId);
        if(!std::getline(input,line)||line!="TYPE Tilemap")return fail("Invalid tilemap cache type: "+reference.assetId);
        if(!std::getline(input,line)||!line.starts_with("FORMAT ")||!std::getline(input,line)||!line.starts_with("SIZE ")||
           !std::getline(input,line)||line!="DATA")return fail("Invalid tilemap cache: "+reference.assetId);
        std::string error;auto map=TilemapSerializer::Load(input,error);
        if(!map)return fail("Cannot decode tilemap "+reference.assetId+": "+error);
        resource.error.clear();resource.map=std::move(map);
        resource.chunks.Update(*resource.map,resource.atlas?&*resource.atlas:nullptr,true);resource.chunks.Flatten(resource.geometry.vertices);
        return &resource;
    }
    const GeometryCommand *Geometry(const AssetReference &id,const Tileset2D *atlas=nullptr) {
        const auto *resolved=Resolve(id);if(!resolved||!resolved->map)return nullptr;
        auto &resource=cache[id.assetId];
        std::optional<Tileset2D> desired=atlas?std::optional{*atlas}:std::nullopt;
        if(resource.atlas!=desired){resource.atlas=desired;
            resource.chunks.Update(*resource.map,atlas,true);resource.chunks.Flatten(resource.geometry.vertices);}
