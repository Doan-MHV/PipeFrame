...
public:
    ...
    void Update(const Tilemap2D& map, const Tileset2D* atlas = nullptr, bool newDocument = false) {
        ...
        const bool layoutChanged = columns != map.Columns() || rows != map.Rows() || cellSize != map.CellSize() ||
                                   origin != map.Origin() || tileset != desired;
        ...
        tileset = desired;
        std::size_t index = 0;
        for (int y = 0; y < rows; y += ChunkSize)
            for (int x = 0; x < columns; x += ChunkSize, ++index) {
                const GridCellRange cells{
                    {x, y}, {std::min(columns - 1, x + ChunkSize - 1), std::min(rows - 1, y + ChunkSize - 1)}};
                ...
                for (const auto& layer : map.Layers()) {
                    ...
                    for (int cy = cells.minimum.row; cy <= cells.maximum.row; ++cy)
                        for (int cx = cells.minimum.column; cx <= cells.maximum.column; ++cx) {
                            const auto id = layer.cells.At({cx, cy});
                            mix(id);
                            if (const auto* tile = map.Definition(id)) {
                                mix(tile->tint.r);
                                mix(tile->tint.g);
                                mix(tile->tint.b);
                                mix(tile->tint.a);
                ...
                if (chunk.fingerprint != hash) {
                    chunk.fingerprint = hash;
                    chunk.geometry = BuildTilemapGeometry(map, cells, atlas);
                    ++rebuilt;
                }
            }
    ...
    void Flatten(std::vector<Vertex2D>& vertices) const {
        ...
        for (const auto& chunk : chunks)
            count += chunk.geometry.vertices.size();
        vertices.clear();
        if (count > vertices.capacity())
            vertices.reserve(std::max(count, vertices.capacity() + vertices.capacity() / 2));
        for (const auto& chunk : chunks)
            vertices.insert(vertices.end(), chunk.geometry.vertices.begin(), chunk.geometry.vertices.end());
    }
