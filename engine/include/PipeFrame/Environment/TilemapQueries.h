...
inline Vector2f LocalDirection(Vector2f p, const Transform2DComponent& t) {
    p = Rotate(p, -t.rotation);
    return {p.x / t.scale.x, p.y / t.scale.y};
}
inline std::optional<std::size_t> SolidLayer(const Tilemap2D& map, GridCoordinate cell, std::uint32_t mask) {
    for (std::size_t layer = 0; layer < map.Layers().size(); ++layer) {
        ...
        const auto* tile = map.Definition(map.Tile(layer, cell));
        if (tile && tile->solid) return layer;
    }
    return std::nullopt;
}
}  // namespace tilemap_query_detail
...
inline std::optional<TilemapQueryHit> RaycastTilemap(const Tilemap2D& map, const Transform2DComponent& transform,
                                                     Vector2f origin, Vector2f direction, float maximumDistance,
                                                     std::uint32_t layerMask = ~std::uint32_t{},
                                                     std::uint64_t objectId = 0,
                                                     std::optional<Rectanglef> localBounds = {}) {
    using namespace tilemap_query_detail;
    const float length = Length(direction);
    if (!Valid(transform) || !std::isfinite(length) || length <= 0 || !std::isfinite(maximumDistance) ||
        maximumDistance < 0)
        return {};
    direction /= length;
    ...
    float entry = 0, end = maximumDistance;
    Vector2f entryNormal{};
    if (localBounds) {
        const auto bounds = *localBounds;
        if (!std::isfinite(bounds.position.x) || !std::isfinite(bounds.position.y) || !std::isfinite(bounds.size.x) ||
            !std::isfinite(bounds.size.y) || bounds.size.x <= 0 || bounds.size.y <= 0)
            return {};
        ...
        if (!clip(localOrigin.x, localDirection.x, bounds.position.x, bounds.position.x + bounds.size.x, {-1, 0}) ||
            !clip(localOrigin.y, localDirection.y, bounds.position.y, bounds.position.y + bounds.size.y, {0, -1}) ||
            end < 0)
            return {};
        if (entry == 0 && ((localOrigin.x >= bounds.position.x + bounds.size.x && localDirection.x >= 0) ||
                           (localOrigin.y >= bounds.position.y + bounds.size.y && localDirection.y >= 0)))
            return {};
        localOrigin += localDirection * entry;
    }
    const auto hit = RaycastGrid(
        localOrigin, localDirection, (end - entry) * factor, map.Origin(), map.CellSize(), map.Columns(), map.Rows(),
        [&](GridCoordinate cell) {
            ...
            if ((localOrigin.x == face.x && localDirection.x < 0) ||
                ...
                (localOrigin.y == face.y + map.CellSize() && localDirection.y > 0))
                return false;
            if (localBounds) {
                ...
                if (a.x >= end.x || a.y >= end.y || a.x + map.CellSize() <= localBounds->position.x ||
                    a.y + map.CellSize() <= localBounds->position.y)
                    return false;
            }
    ...
    return TilemapQueryHit{objectId, layer,   hit->cell, map.Tile(layer, hit->cell), origin + direction * distance,
                           normal,   distance};
}
...
inline std::vector<TilemapQueryHit> OverlapCircleTilemap(const Tilemap2D& map, const Transform2DComponent& transform,
                                                         Circle2D circle, std::uint32_t layerMask = ~std::uint32_t{},
                                                         std::uint64_t objectId = 0,
                                                         std::optional<Rectanglef> localBounds = {}) {
    using namespace tilemap_query_detail;
    std::vector<TilemapQueryHit> result;
    if (!Valid(transform) || !std::isfinite(circle.radius) || circle.radius < 0 || !std::isfinite(circle.center.x) ||
    ...
    if (!std::isfinite(local.x) || !std::isfinite(local.y) || !std::isfinite(radius.x) || !std::isfinite(radius.y))
        return result;
    const auto index = [&](double value, double origin, int count) {
        return int(std::clamp(std::floor((value - origin) / map.CellSize()), -1.0, double(count)));
    };
    const int left =
        std::max(0, index(std::nextafter(double(local.x) - radius.x, -std::numeric_limits<double>::infinity()),
                          map.Origin().x, map.Columns()));
    const int right = std::min(map.Columns() - 1, index(double(local.x) + radius.x, map.Origin().x, map.Columns()));
    const int top =
        std::max(0, index(std::nextafter(double(local.y) - radius.y, -std::numeric_limits<double>::infinity()),
                          map.Origin().y, map.Rows()));
    const int bottom = std::min(map.Rows() - 1, index(double(local.y) + radius.y, map.Origin().y, map.Rows()));
    for (int y = top; y <= bottom; ++y)
        for (int x = left; x <= right; ++x) {
            const GridCoordinate cell{x, y};
            const auto layer = SolidLayer(map, cell, layerMask);
            if (!layer) continue;
            auto a = map.Origin() + Vector2f{float(x), float(y)} * map.CellSize(),
                 b = a + Vector2f{map.CellSize(), map.CellSize()};
            if (localBounds) {
                const auto bounds = *localBounds;
                if (!std::isfinite(bounds.position.x) || !std::isfinite(bounds.position.y) ||
                    !std::isfinite(bounds.size.x) || !std::isfinite(bounds.size.y) || bounds.size.x <= 0 ||
                    bounds.size.y <= 0)
                    return {};
            ...
            const float distance = Length(center - closest);
