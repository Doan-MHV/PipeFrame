...
public:
    ...
    void ReplaceWith(const Tilemap2D& other) {
        const auto next = std::max(revision, other.revision) + 1;
        *this = other;
        revision = next;
    }
    const std::string& Tileset() const { return tileset; }
    void SetTileset(std::string id) {
        if (tileset != id) {
            tileset = std::move(id);
            ++revision;
        }
    ...
    std::optional<GridCoordinate> CellAt(Vector2f local) const {
        ...
        GridCoordinate cell{int(std::floor((local.x - origin.x) / cellSize)),
                            int(std::floor((local.y - origin.y) / cellSize))};
    ...
    void DefineTile(TileDefinition2D tile) {
        ...
        definitions.insert_or_assign(tile.id, tile);
        ++revision;
    }
    ...
    std::size_t AddLayer(std::string name) {
        if (name.empty() || layers.size() + dataLayers.size() >= 32 ||
            std::size_t(columns) * rows > MaxCells / (layers.size() + dataLayers.size() + 1))
            throw std::invalid_argument("Invalid layer name or tilemap cell budget exceeded");
        layers.push_back({std::move(name), true, true, false, Grid2D<TileId>(columns, rows)});
        ++revision;
    ...
    void AddDataLayer(std::string id, double minimum = 0, double maximum = 1) {
        if (id.empty() || id.size() > 128 || DataLayer(id) || !std::isfinite(minimum) || !std::isfinite(maximum) ||
            minimum > 0 || maximum < 0 || minimum >= maximum || layers.size() + dataLayers.size() >= 32 ||
            std::size_t(columns) * rows > MaxCells / (layers.size() + dataLayers.size() + 1))
            throw std::invalid_argument("Invalid data layer or cell budget exceeded");
        dataLayers.push_back({std::move(id), minimum, maximum, false, Grid2D<double>(columns, rows)});
        ++revision;
    }
    bool SetData(std::string_view id, GridCoordinate cell, double value) {
        for (auto& layer : dataLayers)
            if (layer.id == id) {
                if (layer.locked || !Contains(cell) || !std::isfinite(value) || value < layer.minimum ||
                    value > layer.maximum)
                    return false;
                if (layer.cells.At(cell) != value) {
                    layer.cells.At(cell) = value;
                    ++revision;
                }
                return true;
            }
    ...
    void RenameLayer(std::size_t index, std::string name) {
        ...
        layers.at(index).name = std::move(name);
        ++revision;
    }
    ...
    void MoveLayer(std::size_t index, std::size_t destination) {
        ...
        auto layer = std::move(layers[index]);
        layers.erase(layers.begin() + index);
        layers.insert(layers.begin() + destination, std::move(layer));
        ++revision;
    }
    ...
    bool SetTile(std::size_t layer, GridCoordinate cell, TileId tile) {
        ...
        if (target.cells.At(cell) != tile) {
            target.cells.At(cell) = tile;
            ++revision;
        }
        return true;
    }
