...
public:
    ...
    virtual std::optional<TilemapQueryHit> RaycastEnvironment(Vector2f origin, Vector2f direction, float distance,
    ...
    virtual std::optional<TilemapQueryHit> SweepEnvironment(Circle2D circle, Vector2f displacement,
                                                            std::uint32_t mask = ~std::uint32_t{},
                                                            std::uint64_t ignoredObject = 0,
                                                            bool ignoreMoving = false) = 0;
};
}  // namespace pipeframe
