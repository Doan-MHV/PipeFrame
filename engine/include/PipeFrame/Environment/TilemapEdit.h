    ...
    bool Apply(Tilemap2D& map, bool forward = true) const {
        if (!dataTarget.empty()) {
            ...
            for (const auto& change : dataChanges) {
                ...
                if (!seen.emplace(change.cell.column, change.cell.row).second || !map.Contains(change.cell) ||
                    !std::isfinite(value) || value < target->minimum || value > target->maximum ||
                    target->cells.At(change.cell) != (forward ? change.before : change.after))
                    return false;
            }
            for (const auto& change : dataChanges)
                map.SetData(dataTarget, change.cell, forward ? change.after : change.before);
            return true;
        }
        ...
        for (const auto& change : changes) {
            ...
            if (!map.Contains(change.cell) || (target && !map.Definition(target)) ||
                map.Tile(layer, change.cell) != (forward ? change.before : change.after))
                return false;
        }
        for (const auto& change : changes)
            map.SetTile(layer, change.cell, forward ? change.after : change.before);
        return true;
    }
};
class TilemapEdit {
public:
    TilemapEdit(const Tilemap2D& map, std::size_t layer, std::optional<Rectanglef> bounds = {}, bool readOnly = false)
        : map(map), layer(layer), revision(map.Revision()), bounds(bounds) {
        if (layer >= map.Layers().size() || (!readOnly && map.Layers()[layer].locked))
            throw std::invalid_argument("Layer is unavailable for painting");
    }
    bool Contains(GridCoordinate cell) const {
        ...
        return bounds->size.x > 0 && bounds->size.y > 0 && start.x < bounds->position.x + bounds->size.x &&
               start.y < bounds->position.y + bounds->size.y && start.x + map.CellSize() > bounds->position.x &&
               start.y + map.CellSize() > bounds->position.y;
    }
    void Sample(GridCoordinate cell, const EnvironmentBrush& brush) {
        ...
        if (!brush.DataTarget().empty()) {
            if (!pending.empty() || (!dataTarget.empty() && dataTarget != brush.DataTarget()))
                throw std::invalid_argument("A stroke may edit only one target");
            ...
            const auto found = dataPending.find(key);
            const double before = target->cells.At(cell);
            const double value = brush.PaintData(cell, found == dataPending.end() ? before : found->second.after);
            if (!std::isfinite(value) || value < target->minimum || value > target->maximum)
                throw std::invalid_argument("Brush returned an invalid data value");
            dataPending[key] = {cell, before, value};
            return;
        }
    ...
    void Line(GridCoordinate a, GridCoordinate b, const EnvironmentBrush& brush, int radius = 0) {
        ...
        if (!clip(-dx, a.column) || !clip(dx, double(map.Columns() - 1) - a.column) || !clip(-dy, a.row) ||
            !clip(dy, double(map.Rows() - 1) - a.row))
            return;
    ...
    void Fill(GridCoordinate seed, const EnvironmentBrush& brush) {
        ...
        while (!queue.empty()) {
            ...
            Sample(cell, brush);
            queue.push({cell.column - 1, cell.row});
            queue.push({cell.column + 1, cell.row});
            queue.push({cell.column, cell.row - 1});
            queue.push({cell.column, cell.row + 1});
        }
    }
    TilemapPatch Preview() const {
        ...
        for (const auto& [key, change] : pending)
            if (change.before != change.after) patch.changes.push_back(change);
        patch.dataTarget = dataTarget;
        for (const auto& [key, change] : dataPending)
            if (change.before != change.after) patch.dataChanges.push_back(change);
        return patch;
    }
