...
inline std::optional<TilemapQueryHit> SweepCircleTilemap(const Tilemap2D &map,
    const Transform2DComponent &transform,Circle2D circle,Vector2f displacement,
    std::uint32_t mask=~std::uint32_t{},std::uint64_t objectId=0,std::optional<Rectanglef> bounds={}) {
    using namespace tilemap_query_detail;
    const float travel=Length(displacement);
    if(!Valid(transform)||!std::isfinite(travel)||travel<=0||!std::isfinite(circle.radius)||circle.radius<0)return {};
    const auto candidates=OverlapCircleTilemap(map,transform,
        {circle.center+displacement*.5f,circle.radius+travel*.5f},mask,objectId,bounds);
    ...
    for(const auto &candidate:candidates){
        ...
        if(bounds){a={std::max(a.x,bounds->position.x),std::max(a.y,bounds->position.y)};
        ...
        if(hit&&hit->fraction<best){
            best=hit->fraction;auto result=candidate;result.distance=hit->fraction*travel;
            result.normal=Rotate(hit->normal,transform.rotation);
            result.point=transform.position+Rotate(hit->point,transform.rotation);nearest=result;
        }
    }
    return nearest;
}
