...
struct TextureBinding {
    const GraphicsResourceService* resources{};
    TextureHandle handle{};
    TextureBinding() = default;
    TextureBinding(std::nullptr_t) {}
    TextureBinding(const GraphicsResourceService& resources, TextureHandle handle)
...
struct ShaderBinding {
    ...
    ShaderBinding(std::nullptr_t) {}
    ShaderBinding(const GraphicsResourceService& resources, ShaderHandle handle)
        : resources(&resources), handle(handle) {}
};
...
public:
    ...
    void Draw(const Vertex2D* vertices, std::size_t count, PrimitiveTopology topology,
    ...
    void DrawRing(Vector2f center, float radius, float thickness, Color color) const {
        ...
        for (std::size_t i = 0; i < 64; ++i) {
            ...
            const Vertex2D outerA{center + a * (radius + thickness), color, {}},
                outerB{center + b * (radius + thickness), color, {}};
            const std::array quad{innerA, outerA, outerB, innerA, outerB, innerB};
            for (std::size_t j = 0; j < 6; ++j)
                vertices[i * 6 + j] = quad[j];
        }
